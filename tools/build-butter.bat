@echo off
REM ---------------------------------------------------------------
REM  Butter (Butter fork) portable Windows build
REM
REM  Uses only project-local tools, nothing is installed system wide:
REM    .tools\venv            -> meson + ninja (python venv)
REM    butter\cutter-deps\qt  -> portable Qt 6   (scripts\fetch_deps.sh)
REM    butter\cutter-deps\pyside -> portable PySide6 + shiboken (same script)
REM    (the dependency folder keeps its upstream `cutter-deps` name: it is
REM     published by the Butter project and is downloaded as-is)
REM
REM  Builds the *full* feature set that Butter's own Windows CI builds, so the
REM  result is a complete Butter rather than a stripped one:
REM
REM    Python console + Python plugins     (BUTTER_ENABLE_PYTHON)
REM    Python bindings                     (BUTTER_ENABLE_PYTHON_BINDINGS)
REM    FLIRT signature database            (BUTTER_ENABLE_SIGDB)
REM    Ghidra decompiler                   (BUTTER_PACKAGE_RZ_GHIDRA)
REM    jsdec JS decompiler                 (BUTTER_PACKAGE_JSDEC)
REM    Swift demangler                     (BUTTER_PACKAGE_RZ_LIBSWIFT)
REM    YARA scanning                       (BUTTER_PACKAGE_RZ_LIBYARA)
REM    Silhouette function detection       (BUTTER_PACKAGE_RZ_SILHOUETTE)
REM    Frida dynamic instrumentation       (BUTTER_PACKAGE_RZ_FRIDA)
REM
REM  The optional plugins are cloned and built at install time by Butter's own
...
REM
REM  Usage:  build-butter.bat              configure (once) + build
REM          build-butter.bat configure    only run the CMake configure step
REM          build-butter.bat reconfigure  wipe the CMake cache and configure again
REM          build-butter.bat install      also install the portable app to butter-dist\
REM          build-butter.bat full         configure + build + install in one go
REM ---------------------------------------------------------------
...
set "BUILD=%BUTTER_DIR%\build-butter"
set "DIST=%ROOT%\butter-dist"
set "VCVARS=C:\Program Files\Microsoft Visual Studio\18\Community\VC\Auxiliary\Build\vcvars64.bat"

REM --- feature switches (set to OFF to build a leaner Butter) ---
set "BUTTER_PYTHON=ON"
set "BUTTER_PYTHON_BINDINGS=ON"
set "BUTTER_SIGDB=ON"
set "BUTTER_PLUGINS=ON"
...
if /i "%BUTTER_PYTHON_BINDINGS%"=="ON" (
...
REM rz-frida cutter-plugin step reads CUTTER_DEPS to find Qt. Butter's CI sets it
...
if not exist "%BUILD%\CMakeCache.txt" (
    echo [*] Configuring Butter ^(VS 2026 + portable Qt 6 + PySide6^) ...
    cmake -S "%BUTTER_DIR%" -B "%BUILD%" -G Ninja ^
        -DCMAKE_BUILD_TYPE=Release ^
        -DCMAKE_INSTALL_PREFIX="%DIST%" ^
        -DCMAKE_POLICY_VERSION_MINIMUM=3.5 ^
        -DCMAKE_TOOLCHAIN_FILE="%BUTTER_DIR%\cmake\butter-toolchain.cmake" ^
        -DCMAKE_PREFIX_PATH="%DEPS%\qt;%DEPS%\pyside" ^
        -DBUTTER_USE_BUNDLED_RIZIN=ON ^
        -DBUTTER_ENABLE_PYTHON=%BUTTER_PYTHON% ^
        -DBUTTER_ENABLE_PYTHON_BINDINGS=%BUTTER_PYTHON_BINDINGS% ^
        -DPython3_FIND_REGISTRY=NEVER ^
        -DPython3_FIND_STRATEGY=LOCATION ^
        -DPython_FIND_REGISTRY=NEVER ^
        -DPython_FIND_STRATEGY=LOCATION ^
        -DBUTTER_ENABLE_SIGDB=%BUTTER_SIGDB% ^
        -DBUTTER_ENABLE_PACKAGING=ON ^
        -DBUTTER_PACKAGE_DEPENDENCIES=ON ^
        -DBUTTER_PACKAGE_RZ_GHIDRA=ON ^
        -DBUTTER_PACKAGE_JSDEC=%BUTTER_PLUGINS% ^
        -DBUTTER_PACKAGE_RZ_LIBSWIFT=%BUTTER_PLUGINS% ^
        -DBUTTER_PACKAGE_RZ_LIBYARA=%BUTTER_PLUGINS% ^
        -DBUTTER_PACKAGE_RZ_SILHOUETTE=%BUTTER_PLUGINS% ^
        -DBUTTER_PACKAGE_RZ_FRIDA=%BUTTER_PLUGINS% ^
...
cmake --install "%BUILD%"
if errorlevel 1 (
    echo [!] Install step failed. Re-run "build-butter.bat install" to retry the
    echo     optional plugin bundling; the core app is already installed.
    exit /b 1
)

REM Butter only bundles a Python *runtime* when building a ZIP package, so a plain
REM install leaves python3xx\ with site-packages but no interpreter. Run Butter's own
REM bundler to fetch the matching embeddable Python into the portable folder.
if /i "%BUTTER_PYTHON%"=="ON" (
    if not exist "%DIST%\python312._pth" (
        echo [*] Bundling the portable Python runtime ...
        pushd "%BUILD%"
        powershell -NoProfile -ExecutionPolicy Bypass -File "%BUTTER_DIR%\dist\bundle_python.ps1" x64 "%DIST%"
        if errorlevel 1 echo [!] Python bundling failed; the GUI will fall back to a system Python.
        popd
    )
)

echo [+] Done. Run "%DIST%\butter.exe"
endlocal
