...
void AnalysisTask::runTask()
{
    ...
    // takes the crackme from 423 to 614 functions). Applied before the user's
    // analysis commands run, so Advanced mode can still override them.
    rz_config_set_b(core->config, "analysis.hasnext", true);
    rz_config_set_b(core->config, "analysis.jmp.indir", true);
    rz_config_set_b(core->config, "analysis.jmp.above", true);
    rz_config_set_b(core->config, "analysis.jmp.cref", true);
    rz_config_set_b(core->config, "analysis.jmp.ref", true);
    rz_config_set_i(core->config, "analysis.jmp.tblmax", 2048);
    rz_config_set_b(core->config, "analysis.types.constraint", true);
    rz_config_set_b(core->config, "analysis.refstr", true);
    rz_config_set_b(core->config, "analysis.brokenrefs", true);
    rz_config_set_i(core->config, "analysis.ptrdepth", 4);
