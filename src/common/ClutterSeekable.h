 ...
 */
class CLUTTER_EXPORT ClutterSeekable : public QObject
{
    Q_OBJECT

public:
    explicit ClutterSeekable(QObject *parent = nullptr);
    ~ClutterSeekable();

    /**
     * @brief seek changes current offset.
     * If the seekable is synchronized with Core, then
     * the Core offset will be modified and then the ClutterCore::seekChanged
     * signal will be emitted.
     * In any case, ClutterSeekable::seekableSeekChanged is emitted.
     * @param addr the location to seek at.
     * @param type the type of seek wrt history (Undo, Redo, or New)
     */
    void seek(RVA addr, ClutterCore::SeekHistoryType type = ClutterCore::SeekHistoryType::New)
    {
...
private slots:
    void onCoreSeekChanged(RVA addr, ClutterCore::SeekHistoryType type);
...
private:
     ...
     */
    void updateSeek(RVA addr, ClutterCore::SeekHistoryType type, bool localOnly);

signals:
    void seekableSeekChanged(RVA addr, ClutterCore::SeekHistoryType type);
    void syncChanged();
};
