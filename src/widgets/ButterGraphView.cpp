...
ButterGraphView::ButterGraphView(QWidget *parent)
    : GraphView(parent),
      mFontMetrics(nullptr),
      actionExportGraph(tr("Export Graph"), this),
      graphLayout(GraphView::Layout::GridMedium)
{
    connect(Core(), &ButterCore::graphOptionsChanged, this, &ButterGraphView::refreshView);
    connect(Config(), &Configuration::colorsUpdated, this, &ButterGraphView::colorsUpdatedSlot);
    connect(Config(), &Configuration::fontsUpdated, this, &ButterGraphView::fontsUpdatedSlot);

    initFont();
    updateColors();

    connect(&actionExportGraph, &QAction::triggered, this, &ButterGraphView::showExportDialog);
    ...
    connect(horizontalLayoutAction, &QAction::toggled, this, &ButterGraphView::updateLayout);
...
QPoint ButterGraphView::getTextOffset(int line) const
{
    return QPoint(padding, padding + line * charHeight);
}

void ButterGraphView::initFont()
{
...
void ButterGraphView::zoom(QPointF mouseRelativePos, double velocity)
{
    const qreal newScale = getViewScale() * std::pow(1.25, velocity);
    setZoom(mouseRelativePos, newScale);
}

void ButterGraphView::setZoom(QPointF mouseRelativePos, double scale)
{
...
void ButterGraphView::zoomIn()
{
    zoom(QPointF(0.5, 0.5), 1);
}

void ButterGraphView::zoomOut()
{
    zoom(QPointF(0.5, 0.5), -1);
}

void ButterGraphView::zoomReset()
{
    setZoom(QPointF(0.5, 0.5), 1);
}

void ButterGraphView::showExportDialog()
{
    showExportGraphDialog("global_funcall", RZ_CORE_GRAPH_TYPE_FUNCALL, RVA_INVALID);
}

void ButterGraphView::updateColors()
{
...
void ButterGraphView::colorsUpdatedSlot()
{
    updateColors();
    refreshView();
}

GraphLayout::LayoutConfig ButterGraphView::getLayoutConfig()
{
...
void ButterGraphView::updateLayout()
{
...
void ButterGraphView::fontsUpdatedSlot()
{
    initFont();
    refreshView();
}

bool ButterGraphView::event(QEvent *event)
{
    switch (event->type()) {
...
void ButterGraphView::refreshView()
{
    initFont();
    setLayoutConfig(getLayoutConfig());
}

bool ButterGraphView::gestureEvent(QGestureEvent *event)
{
    if (!event) {
        return false;
    }
...
void ButterGraphView::wheelEvent(QWheelEvent *event)
{
...
void ButterGraphView::resizeEvent(QResizeEvent *event)
{
    GraphView::resizeEvent(event);
    emit resized();
}

void ButterGraphView::saveCurrentBlock() {}

void ButterGraphView::restoreCurrentBlock() {}

void ButterGraphView::mousePressEvent(QMouseEvent *event)
{
    GraphView::mousePressEvent(event);
    emit graphMoved();
}

void ButterGraphView::mouseMoveEvent(QMouseEvent *event)
{
    GraphView::mouseMoveEvent(event);
    emit graphMoved();
}

void ButterGraphView::exportGraph(const QString &filePath, GraphExportType exportType,
...
void ButterGraphView::exportRzTextGraph(const QString &filePath, RzCoreGraphType type,
...
bool ButterGraphView::graphIsBitamp(ButterGraphView::GraphExportType type)
{
    switch (type) {
    case GraphExportType::Png:
    case GraphExportType::Jpeg:
...
Q_DECLARE_METATYPE(ButterGraphView::GraphExportType);

void ButterGraphView::showExportGraphDialog(const QString &defaultName, RzCoreGraphType type,
                                            RVA address)
{
