...
ClutterGraphView::ClutterGraphView(QWidget *parent)
    : GraphView(parent),
      mFontMetrics(nullptr),
      actionExportGraph(tr("Export Graph"), this),
      graphLayout(GraphView::Layout::GridMedium)
{
    connect(Core(), &ClutterCore::graphOptionsChanged, this, &ClutterGraphView::refreshView);
    connect(Config(), &Configuration::colorsUpdated, this, &ClutterGraphView::colorsUpdatedSlot);
    connect(Config(), &Configuration::fontsUpdated, this, &ClutterGraphView::fontsUpdatedSlot);

    initFont();
    updateColors();

    connect(&actionExportGraph, &QAction::triggered, this, &ClutterGraphView::showExportDialog);
    ...
    connect(horizontalLayoutAction, &QAction::toggled, this, &ClutterGraphView::updateLayout);
...
QPoint ClutterGraphView::getTextOffset(int line) const
{
    return QPoint(padding, padding + line * charHeight);
}

void ClutterGraphView::initFont()
{
...
void ClutterGraphView::zoom(QPointF mouseRelativePos, double velocity)
{
    const qreal newScale = getViewScale() * std::pow(1.25, velocity);
    setZoom(mouseRelativePos, newScale);
}

void ClutterGraphView::setZoom(QPointF mouseRelativePos, double scale)
{
...
void ClutterGraphView::zoomIn()
{
    zoom(QPointF(0.5, 0.5), 1);
}

void ClutterGraphView::zoomOut()
{
    zoom(QPointF(0.5, 0.5), -1);
}

void ClutterGraphView::zoomReset()
{
    setZoom(QPointF(0.5, 0.5), 1);
}

void ClutterGraphView::showExportDialog()
{
    showExportGraphDialog("global_funcall", RZ_CORE_GRAPH_TYPE_FUNCALL, RVA_INVALID);
}

void ClutterGraphView::updateColors()
{
...
void ClutterGraphView::colorsUpdatedSlot()
{
    updateColors();
    refreshView();
}

GraphLayout::LayoutConfig ClutterGraphView::getLayoutConfig()
{
...
void ClutterGraphView::updateLayout()
{
...
void ClutterGraphView::fontsUpdatedSlot()
{
    initFont();
    refreshView();
}

bool ClutterGraphView::event(QEvent *event)
{
    switch (event->type()) {
...
void ClutterGraphView::refreshView()
{
    initFont();
    setLayoutConfig(getLayoutConfig());
}

bool ClutterGraphView::gestureEvent(QGestureEvent *event)
{
    if (!event) {
        return false;
    }
...
void ClutterGraphView::wheelEvent(QWheelEvent *event)
{
...
void ClutterGraphView::resizeEvent(QResizeEvent *event)
{
    GraphView::resizeEvent(event);
    emit resized();
}

void ClutterGraphView::saveCurrentBlock() {}

void ClutterGraphView::restoreCurrentBlock() {}

void ClutterGraphView::mousePressEvent(QMouseEvent *event)
{
    GraphView::mousePressEvent(event);
    emit graphMoved();
}

void ClutterGraphView::mouseMoveEvent(QMouseEvent *event)
{
    GraphView::mouseMoveEvent(event);
    emit graphMoved();
}

void ClutterGraphView::exportGraph(const QString &filePath, GraphExportType exportType,
...
void ClutterGraphView::exportRzTextGraph(const QString &filePath, RzCoreGraphType type,
...
bool ClutterGraphView::graphIsBitamp(ClutterGraphView::GraphExportType type)
{
    switch (type) {
    case GraphExportType::Png:
    case GraphExportType::Jpeg:
...
Q_DECLARE_METATYPE(ClutterGraphView::GraphExportType);

void ClutterGraphView::showExportGraphDialog(const QString &defaultName, RzCoreGraphType type,
                                            RVA address)
{
