#ifndef BUTTERWIDGET_H
#define BUTTERWIDGET_H
...
/**
 * @brief QDockWidget wrapper for Butter
 *
 * This should serve as a base class for all dock widgets in main window
 */
class BUTTER_EXPORT ButterDockWidget : public QDockWidget
{
    Q_OBJECT

public:
    BUTTER_DEPRECATED("Action will be ignored. Use ButterDockWidget(MainWindow*) instead.")
    ButterDockWidget(MainWindow *parent, QAction *action);

    explicit ButterDockWidget(MainWindow *parent);
    ~ButterDockWidget() override;
     ...
     * @see ButterDockWidget#deserializeViewProperties
     */
     ...
     * @see ButterDockWidget#serializeViewProprties
     */
