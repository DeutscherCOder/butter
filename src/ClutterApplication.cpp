...
#ifdef RZ_GITTAP
#    define CLUTTER_COMPILE_TIME_RZ_VERSION "" RZ_GITTAP
#else
#    define CLUTTER_COMPILE_TIME_RZ_VERSION "" RZ_VERSION
#endif

ClutterApplication::ClutterApplication(int &argc, char **argv) : QApplication(argc, argv)
{
    // Setup application information
    setApplicationVersion(CLUTTER_VERSION_FULL);
#ifdef Q_OS_MACOS
    setWindowIcon(QIcon(":/img/clutter_macos_simple.svg"));
#else
    setWindowIcon(QIcon(":/img/clutter.svg"));
#endif
...
#ifdef Q_OS_WIN
    // Hack to force Clutter load internet connection related DLL's
    QSslSocket s;
    s.sslConfiguration();
#endif // Q_OS_WIN
    ...
    const QString localVersion = CLUTTER_COMPILE_TIME_RZ_VERSION;
    ...
    if (rzversion != localVersion) {
        ...
        msg.setText(QString(QObject::tr("The version used to compile Clutter (%1) does not match "
    ...
    Plugins()->loadPlugins(clOptions.enableClutterPlugins);
...
#if defined(Q_OS_MACOS) && defined(CLUTTER_ENABLE_PACKAGING)
    {
...
#if defined(Q_OS_WIN) && defined(CLUTTER_ENABLE_PACKAGING)
    {
...
ClutterApplication::~ClutterApplication()
{
...
void ClutterApplication::launchNewInstance(const QStringList &args)
{
    QProcess process(this);
    process.setEnvironment(QProcess::systemEnvironment());
    QStringList allArgs;
    if (!clOptions.enableClutterPlugins) {
        allArgs.push_back("--no-clutter-plugins");
    }
...
bool ClutterApplication::event(QEvent *e)
{
...
bool ClutterApplication::loadTranslations()
{
    auto locale = Config()->getCurrLocale();

    bool clutterTrLoaded = false;
    ...
    if (std::find_if(availableTranslations.begin(), availableTranslations.end(),
        ...
        auto *trClutter = new QTranslator;
        auto *trQtBase = new QTranslator;
        auto *trQt = new QTranslator;

        const QStringList &clutterTrPaths = Clutter::getTranslationsDirectories();

        for (const auto &trPath : clutterTrPaths) {
            if (trClutter
                && trClutter->load(locale, QLatin1String("clutter"), QLatin1String("_"), trPath)) {
                installTranslator(trClutter);
                clutterTrLoaded = true;
                trClutter = nullptr;
            }
    ...
    if (!clutterTrLoaded) {
        qWarning() << "Cannot load Clutter's translation for " << locale.name();
    }
    return false;
}

QStringList ClutterApplication::getArgs() const
{
...
bool ClutterApplication::parseCommandLineOptions()
{
    ...
    // "no-cutter-plugins" is kept as a hidden alias for scripts written against
    // upstream Cutter before the rebrand.
    const QCommandLineOption disableClutterPlugins({"no-clutter-plugins", "no-cutter-plugins"},
                                                  QObject::tr("Do not load Clutter plugins"));
    cmdParser.addOption(disableClutterPlugins);
    ...
    ClutterCommandLineOptions opts;
    ...
    if (cmdParser.isSet(disablePlugins)) {
        opts.enableClutterPlugins = false;
        opts.enableRizinPlugins = false;
    }

    if (cmdParser.isSet(disableClutterPlugins)) {
        opts.enableClutterPlugins = false;
    }
...
void ClutterProxyStyle::polish(QWidget *widget)
{
