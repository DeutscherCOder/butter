...
#define Core() (ClutterCore::instance())

class RzCoreLocked;

struct CLUTTER_EXPORT AddrRefs
{
    RVA addr;
    QString mapname;
    QString section;
...
struct CLUTTER_EXPORT RegisterRef
{
    ut64 value;
    AddrRefs ref;
    QString name;
 ...
 */
class CLUTTER_EXPORT ClutterCore : public QObject
{
    Q_OBJECT
...
public:
    explicit ClutterCore(QObject *parent = nullptr);
    ~ClutterCore();
    static ClutterCore *instance();

    void initialize(bool loadPlugins = true);
    void loadClutterRC();
    void loadDefaultClutterRC();
    QDir getClutterRCDefaultDirectory() const;
     ...
     * @note if you want to seek to an address, you should use ClutterCore::seek.
     */
     ...
     * @param address - an address to which Clutter will temporarily seek.
     * @return the output of the command
     */
    ...
    ClutterJson cmdj(const char *str);
    ClutterJson cmdj(const QString &str) { return cmdj(str.toUtf8().constData()); }
    QString cmdTask(const QString &str);

    QString getRizinVersionReadable(const char *program = nullptr);
    QString getVersionInformation();

    ClutterJson parseJson(const char *name, char *res, const char *cmd = nullptr);
    ClutterJson parseJson(const char *name, char *res, const QString &cmd = QString())
    {
    ...
    /* Edition functions */
    ClutterRzIter<RzCoreDecodedBytes> getRzCoreDecodedBytesSingle(RVA addr);
    ...
    /**
     * @brief @see ClutterCore::show(ut64)
     ...
     * The decompiler's parent will be set to this ClutterCore instance, so it will automatically be
     * freed later.
     *
    ...
    /**
     * @brief Get the right RzReg object based on the clutter state (debugging vs emulating)
     */
    RzReg *getReg();
    /**
    ...
    RzCoreLocked lock();
    CLUTTER_DEPRECATED("Use ClutterCore::lock instead")
    RzCoreLocked core();
...
signals:
     ...
     */
    void seekChanged(RVA offset, ClutterCore::SeekHistoryType type = SeekHistoryType::New);
...
private:
    ...
    QVector<QString> getClutterRCFilePaths() const;
...
class CLUTTER_EXPORT RzCoreLocked
{
    ClutterCore *const core;

public:
    explicit RzCoreLocked(ClutterCore *core);
