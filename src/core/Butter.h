...
#define Core() (ButterCore::instance())

class RzCoreLocked;

struct BUTTER_EXPORT AddrRefs
{
    RVA addr;
    QString mapname;
    QString section;
...
struct BUTTER_EXPORT RegisterRef
{
    ut64 value;
    AddrRefs ref;
    QString name;
 ...
 */
class BUTTER_EXPORT ButterCore : public QObject
{
    Q_OBJECT
...
public:
    explicit ButterCore(QObject *parent = nullptr);
    ~ButterCore();
    static ButterCore *instance();

    void initialize(bool loadPlugins = true);
    void loadButterRC();
    void loadDefaultButterRC();
    QDir getButterRCDefaultDirectory() const;
     ...
     * @note if you want to seek to an address, you should use ButterCore::seek.
     */
     ...
     * @param address - an address to which Butter will temporarily seek.
     * @return the output of the command
     */
    ...
    ButterJson cmdj(const char *str);
    ButterJson cmdj(const QString &str) { return cmdj(str.toUtf8().constData()); }
    QString cmdTask(const QString &str);

    QString getRizinVersionReadable(const char *program = nullptr);
    QString getVersionInformation();

    ButterJson parseJson(const char *name, char *res, const char *cmd = nullptr);
    ButterJson parseJson(const char *name, char *res, const QString &cmd = QString())
    {
    ...
    /* Edition functions */
    ButterRzIter<RzCoreDecodedBytes> getRzCoreDecodedBytesSingle(RVA addr);
    ...
    /**
     * @brief @see ButterCore::show(ut64)
     ...
     * The decompiler's parent will be set to this ButterCore instance, so it will automatically be
     * freed later.
     *
    ...
    /**
     * @brief Get the right RzReg object based on the butter state (debugging vs emulating)
     */
    RzReg *getReg();
    /**
    ...
    RzCoreLocked lock();
    BUTTER_DEPRECATED("Use ButterCore::lock instead")
    RzCoreLocked core();
...
signals:
     ...
     */
    void seekChanged(RVA offset, ButterCore::SeekHistoryType type = SeekHistoryType::New);
...
private:
    ...
    QVector<QString> getButterRCFilePaths() const;
...
class BUTTER_EXPORT RzCoreLocked
{
    ButterCore *const core;

public:
    explicit RzCoreLocked(ButterCore *core);
