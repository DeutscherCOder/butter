...
static ClutterCore *uniqueInstance;
...
RzCoreLocked::RzCoreLocked(ClutterCore *core) : core(core)
{
    core->coreMutex.lock();
...
static void clutterREventCallback(RzEvent *, int type, void *user, void *data)
{
    auto core = reinterpret_cast<ClutterCore *>(user);
    core->handleREvent(type, data);
}

ClutterCore::ClutterCore(QObject *parent)
    : QObject(parent)
...
{
    if (uniqueInstance) {
        throw std::logic_error("Only one instance of ClutterCore must exist");
    }
    uniqueInstance = this;
}

ClutterCore *ClutterCore::instance()
{
    return uniqueInstance;
}

void ClutterCore::initialize(bool loadPlugins)
{
    ...
    rz_event_hook(rzCore->ev, RZ_EVENT_ALL, clutterREventCallback, this);
    ...
    // Otherwise Rizin may ask the user for input and Clutter would freeze
...
RzCoreLocked ClutterCore::lock()
{
    return RzCoreLocked(this);
}

RzCoreLocked ClutterCore::core()
{
    return lock();
}

QDir ClutterCore::getClutterRCDefaultDirectory() const
{
    return QStandardPaths::writableLocation(QStandardPaths::AppConfigLocation);
}

QVector<QString> ClutterCore::getClutterRCFilePaths() const
{
    QVector<QString> result;
    result.push_back(QFileInfo(QDir::home(), ".clutterrc").absoluteFilePath());
    const QStringList locations =
            QStandardPaths::standardLocations(QStandardPaths::AppConfigLocation);
    for (auto &location : locations) {
        result.push_back(QFileInfo(QDir(location), ".clutterrc").absoluteFilePath());
    }
    result.push_back(QFileInfo(getClutterRCDefaultDirectory(), "rc")
                             .absoluteFilePath()); // File in config editor is from this path
    return result;
}

void ClutterCore::loadClutterRC()
{
    CORE_LOCK();
    const auto result = getClutterRCFilePaths();
    for (auto &clutterRCFilePath : result) {
        auto clutterRCFileInfo = QFileInfo(clutterRCFilePath);
        if (!clutterRCFileInfo.exists() || !clutterRCFileInfo.isFile()) {
            continue;
        }
        qInfo() << tr("Loading initialization file from ") << clutterRCFilePath;
        rz_core_cmd_file(core, clutterRCFilePath.toUtf8().constData());
        rz_cons_flush();
    }
}

void ClutterCore::loadDefaultClutterRC()
{
    CORE_LOCK();
    auto clutterRCFilePath = QFileInfo(getClutterRCDefaultDirectory(), "rc").absoluteFilePath();
    const auto clutterRCFileInfo = QFileInfo(clutterRCFilePath);
    if (!clutterRCFileInfo.exists() || !clutterRCFileInfo.isFile()) {
        return;
    }
    qInfo() << tr("Loading initialization file from ") << clutterRCFilePath;
...
QList<QString> ClutterCore::sdbList(const QString &path)
{
    CORE_LOCK();
    QList<QString> list = QList<QString>();
    const Sdb *root = sdb_ns_path(core->sdb, path.toUtf8().constData(), 0);
    if (root && root->ns) {
        for (const auto &nsi : ClutterRzList<SdbNs>(root->ns)) {
            list << nsi->name;
        }
    }
    return list;
...
QList<QString> ClutterCore::sdbListKeys(const QString &path)
{
    CORE_LOCK();
    QList<QString> list = QList<QString>();
...
QString ClutterCore::sdbGet(const QString &path, const QString &key)
{
    CORE_LOCK();
...
bool ClutterCore::sdbSet(const QString &path, const QString &key, const QString &val)
{
    CORE_LOCK();
...
QString ClutterCore::sanitizeStringForCommand(QString s)
{
    static const QRegularExpression regexp(";|@");
    return s.replace(regexp, QStringLiteral("_"));
}

QString ClutterCore::cmd(const char *str)
{
    CORE_LOCK();
...
QString ClutterCore::getFunctionExecOut(const std::function<bool(RzCore *)> &fcn, const RVA addr)
{
    CORE_LOCK();
...
bool ClutterCore::isRedirectableDebugee() const
{
    ...
    RzListIter *it;
    RzIODesc *desc;
    ClutterRzListForeach (descs, it, RzIODesc, desc) {
...
bool ClutterCore::isDebugTaskInProgress()
{
    if (debugTask) {
        return true;
    }
...
void ClutterCore::setProfileDirectives(const QString &directives)
{
...
void ClutterCore::setRegisterProfile(const QString &profile)
{
    CORE_LOCK();
...
QString ClutterCore::convertGdbProfile(const QString &profilePath)
{
    return QString::fromUtf8(rz_reg_parse_gdb_profile(profilePath.toUtf8().constData()));
}

QString ClutterCore::getRegisterProfile()
{
    CORE_LOCK();
...
bool ClutterCore::asyncTask(std::function<void *(RzCore *)> fcn, std::shared_ptr<RizinTask> &task)
{
    if (task) {
        return false;
    }
...
void ClutterCore::functionTask(std::function<void *(RzCore *)> fcn)
{
...
QString ClutterCore::cmdRawAt(const char *cmd, RVA address)
{
    QString res;
...
QString ClutterCore::cmdRaw(const char *cmd)
{
    const QString res;
    CORE_LOCK();
...
ClutterJson ClutterCore::cmdj(const char *str)
{
    char *res;
    {
        CORE_LOCK();
...
QString ClutterCore::cmdTask(const QString &str)
{
...
ClutterJson ClutterCore::parseJson(const char *name, char *res, const char *cmd)
{
    ...
    return ClutterJson(doc, std::make_shared<ClutterJsonOwner>(doc, res));
}

QStringList ClutterCore::autocomplete(const QString &cmd, RzLinePromptType promptType)
{
    RzLineBuffer buf;
...
bool ClutterCore::loadFile(const QString &path, ut64 baddr, ut64 mapaddr, int perms, int va,
                          bool loadbin, const QString &forceBinPlugin)
{
    ...
    if (perms & RZ_PERM_W) {
        ...
        for (auto map : ClutterPVector<RzIOMap>(maps)) {
            map->perm |= RZ_PERM_W;
        }
    }
...
bool ClutterCore::tryFile(const QString &path, bool rw)
{
    if (path.isEmpty()) {
...
bool ClutterCore::mapFile(const QString &path, RVA mapaddr)
{
    CORE_LOCK();
...
void ClutterCore::renameFunction(const RVA offset, const QString &newName)
{
    CORE_LOCK();
...
void ClutterCore::delFunction(RVA addr)
{
    CORE_LOCK();
...
void ClutterCore::renameFlag(const QString &old_name, const QString &new_name)
{
    CORE_LOCK();
...
void ClutterCore::renameFunctionVariable(const QString &newName, const QString &oldName,
                                        RVA functionAddress)
{
    CORE_LOCK();
...
void ClutterCore::delFlag(RVA addr)
{
    CORE_LOCK();
...
void ClutterCore::delFlag(const QString &name)
{
    CORE_LOCK();
...
ClutterRzIter<RzCoreDecodedBytes> ClutterCore::getRzCoreDecodedBytesSingle(RVA addr)
{
    CORE_LOCK();
    ut8 buf[128];
    ...
    auto result = ClutterRzIter<RzCoreDecodedBytes>(
            rz_core_analysis_bytes(core, addr, buf, sizeof(buf), 1));
    return result;
}

QString ClutterCore::getInstructionBytes(RVA addr)
{
    auto cdb = getRzCoreDecodedBytesSingle(addr);
    return cdb ? cdb->bytes : "";
}

QString ClutterCore::getInstructionOpcode(RVA addr)
{
    auto cdb = getRzCoreDecodedBytesSingle(addr);
    return cdb ? cdb->opcode : "";
}

void ClutterCore::editInstruction(RVA addr, const QString &inst, bool fillWithNops)
{
    CORE_LOCK();
    if (fillWithNops) {
...
void ClutterCore::nopInstruction(RVA addr)
{
    CORE_LOCK();
    {
...
void ClutterCore::jmpReverse(RVA addr)
{
    CORE_LOCK();
    {
        auto seek = seekTemp(addr);
...
void ClutterCore::editBytes(RVA addr, const QString &bytes)
{
    CORE_LOCK();
...
void ClutterCore::editBytesEndian(RVA addr, const QString &bytes)
{
    CORE_LOCK();
...
void ClutterCore::setToCode(RVA addr)
{
    CORE_LOCK();
...
void ClutterCore::setAsString(RVA addr, int size, StringTypeFormats type)
{
    if (RVA_INVALID == addr) {
        return;
    }
...
void ClutterCore::removeString(RVA addr)
{
    CORE_LOCK();
...
QString ClutterCore::getString(RVA addr)
{
    CORE_LOCK();
...
QString ClutterCore::getString(RVA addr, uint64_t len, RzStrEnc encoding, bool escape_nl)
{
    CORE_LOCK();
...
QString ClutterCore::getMetaString(RVA addr)
{
    CORE_LOCK();
    return rz_meta_get_string(core->analysis, RZ_META_TYPE_STRING, addr);
}

void ClutterCore::setToData(RVA addr, int size, int repeat)
{
    if (size <= 0 || repeat <= 0) {
        return;
    }
...
int ClutterCore::sizeofDataMeta(RVA addr)
{
    ut64 size = 0;
    CORE_LOCK();
...
void ClutterCore::setComment(RVA addr, const QString &cmt)
{
    CORE_LOCK();
...
void ClutterCore::delComment(RVA addr)
{
    CORE_LOCK();
...
QString ClutterCore::getCommentAt(RVA addr)
{
    CORE_LOCK();
    return rz_meta_get_string(core->analysis, RZ_META_TYPE_COMMENT, addr);
}

void ClutterCore::setImmediateBase(const QString &rzBaseName, RVA offset)
{
...
void ClutterCore::setCurrentBits(int bits, RVA offset)
{
    if (offset == RVA_INVALID) {
        offset = getOffset();
    }
...
void ClutterCore::applyStructureOffset(const QString &structureOffset, RVA offset)
{
    if (offset == RVA_INVALID) {
        offset = getOffset();
    }
...
void ClutterCore::seekSilent(ut64 offset)
{
    CORE_LOCK();
    if (offset == RVA_INVALID) {
        return;
...
void ClutterCore::seek(ut64 offset)
{
...
void ClutterCore::showMemoryWidget()
{
    emit showMemoryWidgetRequested();
}

void ClutterCore::seekAndShow(ut64 offset)
{
    seek(offset);
    emit showAddressRequested(offset);
}

void ClutterCore::seekAndShow(const QString &offset)
{
    seek(offset);
    emit showAddressRequested(math(offset));
}

void ClutterCore::seek(const QString &thing)
{
    CORE_LOCK();
...
void ClutterCore::updateSeek(SeekHistoryType type)
{
    emit seekChanged(getOffset(), type);
}

RVA ClutterCore::prevOpAddr(RVA startAddr, int count)
{
    CORE_LOCK();
    return rz_core_prevop_addr_force(core, startAddr, count);
}

RVA ClutterCore::nextOpAddr(RVA startAddr, int count)
{
    CORE_LOCK();
...
RVA ClutterCore::getOffset()
{
    return rzCore->offset;
}

void ClutterCore::applySignature(const QString &filepath)
{
    CORE_LOCK();
    int oldCnt, newCnt;
...
void ClutterCore::createSignature(const QString &filepath)
{
    CORE_LOCK();
    ut32 nModules = 0;
...
bool ClutterCore::isValidInputNumValue(const QString &expression)
{
    CORE_LOCK();
    return rz_is_valid_input_num_value(core ? core->num : nullptr, expression.toUtf8().constData());
}

ut64 ClutterCore::math(const QString &expr)
{
    CORE_LOCK();
    return rz_num_math(core ? core->num : nullptr, expr.toUtf8().constData());
}

ut64 ClutterCore::num(const QString &expr)
{
    CORE_LOCK();
    return rz_num_get(core ? core->num : nullptr, expr.toUtf8().constData());
}

QString ClutterCore::itoa(ut64 num, int rdx)
{
    return QString::number(num, rdx);
}

void ClutterCore::setConfig(const char *k, const char *v)
{
    CORE_LOCK();
    rz_config_set(core->config, k, v);
}

void ClutterCore::setConfig(const QString &k, const char *v)
{
    CORE_LOCK();
    rz_config_set(core->config, k.toUtf8().constData(), v);
}

void ClutterCore::setConfig(const char *k, const QString &v)
{
    CORE_LOCK();
    rz_config_set(core->config, k, v.toUtf8().constData());
}

AddressTypeHint ClutterCore::getAddressType(RVA addr)
{
...
void ClutterCore::setConfig(const char *k, int v)
{
    CORE_LOCK();
    rz_config_set_i(core->config, k, static_cast<ut64>(v));
}

void ClutterCore::setConfig(const char *k, ut64 v)
{
    CORE_LOCK();
    rz_config_set_integer(core->config, k, v);
}

void ClutterCore::setConfig(const char *k, bool v)
{
    CORE_LOCK();
    rz_config_set_i(core->config, k, v ? 1 : 0);
}

void ClutterCore::setConfig(const char *k, const RzInterval &itv)
{
    CORE_LOCK();
    rz_config_set_interval(core->config, k, itv);
}

void ClutterCore::setConfig(const char *k, const QStringList &list)
{
    CORE_LOCK();
...
int ClutterCore::getConfigi(const char *k)
{
    CORE_LOCK();
    return static_cast<int>(rz_config_get_i(core->config, k));
}

ut64 ClutterCore::getConfigut64(const char *k)
{
    CORE_LOCK();
    return rz_config_get_i(core->config, k);
}

bool ClutterCore::getConfigb(const char *k)
{
    CORE_LOCK();
    return rz_config_get_i(core->config, k) != 0;
}

RzInterval ClutterCore::getConfigItv(const char *k)
{
    CORE_LOCK();
    return rz_config_get_interval(core->config, k);
}

QSet<QString> ClutterCore::getConfigSet(const char *k)
{
    CORE_LOCK();
...
QString ClutterCore::getConfigDescription(const char *k)
{
    CORE_LOCK();
...
void ClutterCore::triggerRefreshAll()
{
    emit refreshAll();
}

void ClutterCore::triggerAsmOptionsChanged()
{
    emit asmOptionsChanged();
}

void ClutterCore::triggerGraphOptionsChanged()
{
    emit graphOptionsChanged();
}

void ClutterCore::triggerDebugOptionsChanged()
{
    emit debugOptionsChanged();
}

void ClutterCore::triggerAnalysisOptionsChanged()
{
    emit analysisOptionsChanged();
}

void ClutterCore::triggerSymbolsOptionsChanged()
{
    emit symbolsOptionsChanged();
}

void ClutterCore::message(const QString &msg, bool debug)
{
    if (msg.isEmpty()) {
        return;
    }
...
QString ClutterCore::getConfig(const char *k)
{
    CORE_LOCK();
    return { rz_config_get(core->config, k) };
}

QSet<QString> ClutterCore::getConfigOptions(const char *k)
{
    CORE_LOCK();
    const RzConfigNode *node = rz_config_node_get(core->config, k);
    ...
    QSet<QString> res;
    for (auto it = ClutterRzIter<const char *>(rz_set_s_as_iter(node->options)); it; ++it) {
        res << QString::fromUtf8(*it);
    }
    return res;
}

void ClutterCore::setConfig(const char *k, const QVariant &v)
{
...
void ClutterCore::setCPU(const QString &arch, const QString &cpu, int bits)
{
    if (arch != nullptr) {
...
void ClutterCore::setEndianness(bool big)
{
    setConfig("cfg.bigendian", big);
}

QByteArray ClutterCore::assemble(const QString &code)
{
    CORE_LOCK();
...
QString ClutterCore::disassemble(const QByteArray &data)
{
    CORE_LOCK();
...
QString ClutterCore::disassembleSingleInstruction(RVA addr)
{
    auto cdb = getRzCoreDecodedBytesSingle(addr);
    return QString(cdb->disasm).simplified();
}

RzAnalysisFunction *ClutterCore::functionIn(ut64 addr)
{
    CORE_LOCK();
...
RzAnalysisFunction *ClutterCore::functionAt(ut64 addr)
{
    CORE_LOCK();
    return rz_analysis_get_function_at(core->analysis, addr);
}

RVA ClutterCore::getFunctionStart(RVA addr)
{
    const auto core = Core()->lock();
...
RVA ClutterCore::getFunctionEnd(RVA addr)
{
    const auto core = Core()->lock();
    const RzAnalysisFunction *fcn = Core()->functionIn(addr);
    return fcn ? fcn->addr : RVA_INVALID;
...
RVA ClutterCore::getLastFunctionInstruction(RVA addr)
{
    const auto core = Core()->lock();
    const RzAnalysisFunction *fcn = Core()->functionIn(addr);
    if (!fcn) {
...
QString ClutterCore::flagAt(RVA addr, bool getClosestFlag)
{
    CORE_LOCK();
...
void ClutterCore::createFunctionAt(RVA addr)
{
    createFunctionAt(addr, "");
}

void ClutterCore::createFunctionAt(RVA addr, QString name)
{
...
RVA ClutterCore::getOffsetJump(RVA addr)
{
    auto cdb = getRzCoreDecodedBytesSingle(addr);
    return cdb ? cdb->an_op.jump : RVA_INVALID;
}

QList<Decompiler *> ClutterCore::getDecompilers()
{
    return decompilers;
}

Decompiler *ClutterCore::getDecompilerById(const QString &id)
{
...
bool ClutterCore::registerDecompiler(Decompiler *decompiler)
{
...
ClutterJson ClutterCore::getSignatureInfo()
{
    CORE_LOCK();
...
bool ClutterCore::existsFileInfo()
{
    CORE_LOCK();
...
RefDescription ClutterCore::formatRefDesc(const std::shared_ptr<AddrRefs> &refItem)
{
    RefDescription desc;
...
QList<RegisterRef> ClutterCore::getRegisterRefs(int depth)
{
    ...
    RzListIter *it;
    RzRegItem *ri;
    ClutterRzListForeach (ritems, it, RzRegItem, ri) {
        RegisterRef reg;
...
QList<AddrRefs> ClutterCore::getStack(int size, int depth)
{
...
AddrRefs ClutterCore::getAddrRefs(RVA addr, int depth)
{
    AddrRefs refs;
...
QVector<Chunk> ClutterCore::getHeapChunks(RVA arena_addr)
{
    CORE_LOCK();
    ...
    ClutterRzListForeach (chunks, iter, RzHeapChunkListItem, data) {
        Chunk chunk;
...
int ClutterCore::getArchBits()
{
    CORE_LOCK();
    return core->dbg->bits;
}

QVector<Arena> ClutterCore::getArenas()
{
    CORE_LOCK();
    ...
    ClutterRzListForeach (arenas, iter, RzArenaListItem, data) {
        Arena arena;
...
RzHeapChunkSimple *ClutterCore::getHeapChunk(ut64 addr)
{
    CORE_LOCK();
    return rz_heap_chunk(core, addr);
}

QVector<RzHeapBin *> ClutterCore::getHeapBins(ut64 arena_addr)
{
    CORE_LOCK();
    ...
    RzListIter *iter;
    RzHeapBin *bin;
    ClutterRzListForeach (tcacheBins, iter, RzHeapBin, bin) {
        if (!bin) {
            continue;
        }
        if (!rz_list_length(bin->chunks)) {
...
bool ClutterCore::writeHeapChunk(RzHeapChunkSimple *chunk_simple)
{
    CORE_LOCK();
    return rz_heap_write_chunk(core, chunk_simple);
}

QList<VariableDescription> ClutterCore::getVariables(RVA at)
{
    ...
    for (auto var : ClutterPVector<RzAnalysisVar>(&fcn->vars)) {
...
QList<GlobalDescription> ClutterCore::getAllGlobals()
{
    CORE_LOCK();
    RzListIter *it;
    ...
    if (core && core->analysis) {
        ...
        if (typedb) {
            ...
            ClutterRzListForeach (globals, it, RzAnalysisVarGlobal, glob) {
...
QVector<RegisterRefValueDescription> ClutterCore::getRegisterRefValues()
{
...
RegisterRefValueDescription ClutterCore::getRegisterRefValue(const QString &regName)
{
    RegisterRefValueDescription desc;
    CORE_LOCK();
...
QString ClutterCore::getRegisterName(const QString &registerRole)
{
    if (!currentlyDebugging) {
        return "";
    }
...
RVA ClutterCore::getProgramCounterValue()
{
    if (currentlyDebugging) {
        CORE_LOCK();
...
void ClutterCore::setRegister(const QString &regName, const QString &regValue)
{
    if (!currentlyDebugging) {
        return;
    }
...
void ClutterCore::setCurrentDebugThread(int tid)
{
    if (!asyncTask(
                [=](RzCore *core) {
...
void ClutterCore::setCurrentDebugProcess(int pid)
{
    if (!currentlyDebugging
        || !asyncTask(
                [=](RzCore *core) {
...
void ClutterCore::startEmulation()
{
    if (!currentlyDebugging) {
        offsetPriorDebugging = getOffset();
    }
...
void ClutterCore::attachRemote(const QString &uri)
{
    if (!currentlyDebugging) {
        offsetPriorDebugging = getOffset();
    }
...
void ClutterCore::attachDebug(int pid)
{
    if (!currentlyDebugging) {
        offsetPriorDebugging = getOffset();
    }
...
void ClutterCore::suspendDebug()
{
    debugTask->breakTask();
    debugTask->joinTask();
}

void ClutterCore::stopDebug()
{
    if (!currentlyDebugging) {
        return;
    }
...
void ClutterCore::syncAndSeekProgramCounter()
{
    seekAndShow(getProgramCounterValue());
    emit registersChanged();
}

void ClutterCore::continueDebug()
{
    if (!currentlyDebugging) {
        return;
    }
...
void ClutterCore::continueBackDebug()
{
    if (!currentlyDebugging) {
        return;
    }
...
void ClutterCore::continueUntilDebug(ut64 offset)
{
    if (!currentlyDebugging) {
        return;
    }
...
void ClutterCore::continueUntilCall()
{
    if (!currentlyDebugging) {
        return;
    }
...
void ClutterCore::continueUntilSyscall()
{
    if (!currentlyDebugging) {
        return;
    }
...
void ClutterCore::stepOverDebug()
{
    if (!currentlyDebugging) {
        return;
    }
...
void ClutterCore::stepBackDebug()
{
    if (!currentlyDebugging) {
        return;
    }
...
QStringList ClutterCore::getDebugPlugins()
{
    QStringList plugins;
    CORE_LOCK();
    ClutterHtSP<RzDebugPlugin>(core->dbg->plugins)
...
QString ClutterCore::getActiveDebugPlugin()
{
    return getConfig("dbg.backend");
}

void ClutterCore::setDebugPlugin(const QString &plugin)
{
    setConfig("dbg.backend", plugin);
}

void ClutterCore::startTraceSession()
{
...
void ClutterCore::stopTraceSession()
{
    if (!currentlyDebugging || !currentlyTracing) {
        return;
    }
...
void ClutterCore::toggleBreakpoint(RVA addr)
{
    CORE_LOCK();
...
void ClutterCore::addBreakpoint(const BreakpointDescription &config)
{
    CORE_LOCK();
...
void ClutterCore::updateBreakpoint(int index, const BreakpointDescription &config)
{
    CORE_LOCK();
...
void ClutterCore::delBreakpoint(RVA addr)
{
    CORE_LOCK();
...
void ClutterCore::delAllBreakpoints()
{
    CORE_LOCK();
...
void ClutterCore::enableBreakpoint(RVA addr)
{
    CORE_LOCK();
...
void ClutterCore::disableBreakpoint(RVA addr)
{
    CORE_LOCK();
...
void ClutterCore::setBreakpointTrace(int index, bool enabled)
{
    CORE_LOCK();
...
int ClutterCore::breakpointIndexAt(RVA addr)
{
    CORE_LOCK();
    return rz_bp_get_index_at(core->dbg->bp, addr);
}

BreakpointDescription ClutterCore::getBreakpointAt(RVA addr)
{
    CORE_LOCK();
...
QList<BreakpointDescription> ClutterCore::getBreakpoints()
{
    CORE_LOCK();
...
QList<RVA> ClutterCore::getBreakpointsAddresses()
{
...
QList<RVA> ClutterCore::getBreakpointsInFunction(RVA funcAddr)
{
...
bool ClutterCore::isBreakpoint(const QList<RVA> &breakpoints, RVA addr)
{
    return breakpoints.contains(addr);
}

QList<ThreadDescription> ClutterCore::getProcessThreads(int pid)
{
    CORE_LOCK();
    auto dbg = core->dbg;
    ...
    ClutterRzListForeach (list, iter, RzDebugPid, p) {
...
QList<ProcessDescription> ClutterCore::getProcesses(int pid)
{
    CORE_LOCK();
    auto dbg = core->dbg;
    if (!dbg || !dbg->cur || !dbg->cur->threads) {
...
QList<MemoryMapDescription> ClutterCore::getMemoryMap()
{
    CORE_LOCK();
    ...
    RzListIter *it;
    RzDebugMap *map;
    ClutterRzListForeach (list0, it, RzDebugMap, map) {
...
void ClutterCore::setGraphEmpty(bool empty)
{
    emptyGraph = empty;
}

bool ClutterCore::isGraphEmpty() const
{
    return emptyGraph;
}

void ClutterCore::getRegs()
{
    const auto core = Core()->lock();
    this->regs = {};
    ...
    for (const auto &r : ClutterRzList<RzRegItem>(rs)) {
        this->regs.push_back(r->name);
    }
}

void ClutterCore::setSettings()
{
    setConfig("scr.interactive", false);
...
QList<RVA> ClutterCore::getSeekHistory()
{
    CORE_LOCK();
    QList<RVA> ret;
    RzListIter *it;
...
QStringList ClutterCore::getAsmPluginNames()
{
    CORE_LOCK();
    QStringList ret;
    ClutterHtSP<RzAsmPlugin>(rz_asm_get_plugins(core->rasm))
...
QStringList ClutterCore::getAnalysisPluginNames()
{
    CORE_LOCK();
    QStringList ret;
    ClutterHtSP<RzAnalysisPlugin>(rz_analysis_get_plugins(core->analysis))
...
QList<RzBinPluginDescription> ClutterCore::getBinPluginDescriptions(bool bin, bool xtr)
{
    CORE_LOCK();
    QList<RzBinPluginDescription> ret;
    if (bin) {
        ClutterHtSP<RzBinPlugin>(core->bin->plugins)
    ...
    if (xtr) {
        ClutterHtSP<RzBinXtrPlugin>(core->bin->binxtrs)
...
QList<RzIOPluginDescription> ClutterCore::getRIOPluginDescriptions()
{
    CORE_LOCK();
    QList<RzIOPluginDescription> ret;
    ClutterHtSP<RzIOPlugin>(core->io->plugins)
...
QList<RzCorePluginDescription> ClutterCore::getRCorePluginDescriptions()
{
    CORE_LOCK();
    QList<RzCorePluginDescription> ret;
    ClutterHtSP<RzCorePlugin>(core->plugins)
...
QList<RzAsmPluginDescription> ClutterCore::getRAsmPluginDescriptions()
{
    CORE_LOCK();
...
QList<FunctionDescription> ClutterCore::getAllFunctions()
{
    CORE_LOCK();
    ...
    ClutterRzListForeach (fcns, iter, RzAnalysisFunction, fcn) {
...
QList<ImportDescription> ClutterCore::getAllImports()
{
    CORE_LOCK();
    ...
    for (const auto &import : ClutterPVector<RzBinImport>(imports)) {
...
QList<ExportDescription> ClutterCore::getAllExports()
{
    CORE_LOCK();
    const RzBinFile *bf = rz_bin_cur(core->bin);
    if (!bf) {
    ...
    for (const auto &symbol : ClutterPVector<RzBinSymbol>(symbols)) {
...
QList<SymbolDescription> ClutterCore::getAllSymbols()
{
    CORE_LOCK();
    const RzBinFile *bf = rz_bin_cur(core->bin);
    if (!bf) {
    ...
    const RzPVector *symbols = rz_bin_object_get_symbols(bf->o);
    if (symbols) {
        for (const auto &bs : ClutterPVector<RzBinSymbol>(symbols)) {
    ...
    if (symbols) {
        ...
        int n = 0;
        for (const auto &entry : ClutterPVector<RzBinAddr>(entries)) {
            SymbolDescription symbol;
...
QList<HeaderDescription> ClutterCore::getAllHeaders()
{
    CORE_LOCK();
    const RzBinFile *bf = rz_bin_cur(core->bin);
    if (!bf) {
    ...
    for (auto field : ClutterPVector<RzBinField>(fields)) {
...
QList<FlirtDescription> ClutterCore::getSignaturesDB()
{
    CORE_LOCK();
...
QList<CommentDescription> ClutterCore::getAllComments(const QString &filterType)
{
    CORE_LOCK();
...
QList<RelocDescription> ClutterCore::getAllRelocs()
{
    CORE_LOCK();
...
QList<StringDescription> ClutterCore::getAllStrings(bool raw)
{
    CORE_LOCK();
    ...
    for (const auto &str : ClutterPVector<RzBinString>(strings)) {
...
QList<FlagspaceDescription> ClutterCore::getAllFlagspaces()
{
    CORE_LOCK();
...
QList<FlagDescription> ClutterCore::getAllFlags(const QString &flagspace)
{
    CORE_LOCK();
...
SectionDescription ClutterCore::getSectionAtAddress(RVA addr)
{
    CORE_LOCK();
...
QList<SectionDescription> ClutterCore::getAllSections()
{
    CORE_LOCK();
    ...
    for (const auto &sect : ClutterPVector<RzBinSection>(sects)) {
...
QStringList ClutterCore::getSectionList()
{
    CORE_LOCK();
    QStringList ret;
...
QList<SegmentDescription> ClutterCore::getAllSegments()
{
    CORE_LOCK();
    const RzBinFile *bf = rz_bin_cur(core->bin);
    if (!bf) {
    ...
    for (const auto &segment : ClutterPVector<RzBinSection>(segments)) {
...
QList<EntrypointDescription> ClutterCore::getAllEntrypoint()
{
    CORE_LOCK();
    const RzBinFile *bf = rz_bin_cur(core->bin);
    if (!bf) {
...
QList<BinClassDescription> ClutterCore::getAllClassesFromBin()
{
    CORE_LOCK();
    const RzBinFile *bf = rz_bin_cur(core->bin);
    if (!bf) {
    ...
    RzBinSymbol *sym;
    RzBinClassField *f;
    for (const auto &c : ClutterPVector<RzBinClass>(cs)) {
        BinClassDescription classDescription;
        classDescription.name = c->name;
        classDescription.addr = c->addr;
        ClutterRzListForeach (c->methods, iter2, RzBinSymbol, sym) {
        ...
        ClutterRzListForeach (c->fields, iter3, RzBinClassField, f) {
...
QList<BinClassDescription> ClutterCore::getAllClassesFromFlags()
{
...
QList<QString> ClutterCore::getAllAnalysisClasses(bool sorted)
{
    CORE_LOCK();
    QList<QString> ret;
    ...
    for (const auto &kv : ClutterPVector<SdbKv>(l.get())) {
        ret.append(QString::fromUtf8(reinterpret_cast<const char *>(kv->base.key)));
    }
    return ret;
}

QList<AnalysisMethodDescription> ClutterCore::getAnalysisClassMethods(const QString &cls)
{
    CORE_LOCK();
    ...
    ClutterRzVectorForeach(meths, meth, RzAnalysisMethod)
    {
...
QList<AnalysisBaseClassDescription> ClutterCore::getAnalysisClassBaseClasses(const QString &cls)
{
    CORE_LOCK();
    ...
    ClutterRzVectorForeach(bases, base, RzAnalysisBaseClass)
    {
...
QList<AnalysisVTableDescription> ClutterCore::getAnalysisClassVTables(const QString &cls)
{
    CORE_LOCK();
    ...
    ClutterRzVectorForeach(vtables, vtable, RzAnalysisVTable)
    {
...
void ClutterCore::createNewClass(const QString &cls)
{
    CORE_LOCK();
    rz_analysis_class_create(core->analysis, cls.toUtf8().constData());
}

void ClutterCore::renameClass(const QString &oldName, const QString &newName)
{
    CORE_LOCK();
...
void ClutterCore::deleteClass(const QString &cls)
{
    CORE_LOCK();
    rz_analysis_class_delete(core->analysis, cls.toUtf8().constData());
}

bool ClutterCore::getAnalysisMethod(const QString &cls, const QString &meth,
                                   AnalysisMethodDescription *desc)
{
    CORE_LOCK();
...
void ClutterCore::setAnalysisMethod(const QString &className, const AnalysisMethodDescription &meth)
{
    CORE_LOCK();
    RzAnalysisMethod analysisMeth;
...
void ClutterCore::renameAnalysisMethod(const QString &className, const QString &oldMethodName,
...
QList<ResourcesDescription> ClutterCore::getAllResources()
{
    CORE_LOCK();
    const RzBinFile *bf = rz_bin_cur(core->bin);
    if (!bf) {
    ...
    for (const auto &resource : ClutterPVector<RzBinResource>(resources)) {
...
QList<VTableDescription> ClutterCore::getAllVTables()
{
    CORE_LOCK();
    ...
    ClutterRzListForeach (vtables, iter, RVTableInfo, table) {
        VTableDescription tableDesc;
        tableDesc.addr = table->saddr;
        ClutterRzVectorForeach(&table->methods, method, RVTableMethodInfo)
        {
...
QList<BacktraceDescription> ClutterCore::getAllBacktraces()
{
    ...
    RzListIter *iter;
    RzBacktrace *bt;
    ClutterRzListForeach (list, iter, RzBacktrace, bt) {
...
QList<EvaluableVarDescription> ClutterCore::getAllEvaluableVars()
{
    ...
    ClutterHtSP<RzConfig>(core->plugin_configs)
...
QList<QString> ClutterCore::getAllEvaluableVarSpaces()
{
    CORE_LOCK();
    ...
    RzListIter *iter;
    char *space;
    ClutterRzListForeach (spaces, iter, char, space) {
...
QList<TypeDescription> ClutterCore::getAllTypes()
{
...
QList<TypeDescription> ClutterCore::getBaseType(RzBaseTypeKind kind, const char *category)
{
    CORE_LOCK();
    QList<TypeDescription> types;
    ...
    ClutterRzListForeach (ts, iter, RzBaseType, type) {
        TypeDescription exp;
...
QList<TypeDescription> ClutterCore::getAllPrimitiveTypes()
{
    return getBaseType(RZ_BASE_TYPE_KIND_ATOMIC, "Primitive");
}

QList<TypeDescription> ClutterCore::getAllUnions()
{
    return getBaseType(RZ_BASE_TYPE_KIND_UNION, "Union");
}

QList<TypeDescription> ClutterCore::getAllStructs()
{
    return getBaseType(RZ_BASE_TYPE_KIND_STRUCT, "Struct");
}

QList<TypeDescription> ClutterCore::getAllEnums()
{
    return getBaseType(RZ_BASE_TYPE_KIND_ENUM, "Enum");
}

QList<TypeDescription> ClutterCore::getAllTypedefs()
{
    return getBaseType(RZ_BASE_TYPE_KIND_TYPEDEF, "Typedef");
}

QList<QString> ClutterCore::getAllTypeClasses()
{
...
QString ClutterCore::getTypeAsC(const QString &name)
{
    CORE_LOCK();
...
bool ClutterCore::typeExists(const QString &typeName)
{
    CORE_LOCK();
    return rz_type_exists(rz_analysis_get_type_db(core->analysis), typeName.toUtf8().constData());
}

bool ClutterCore::isAddressMapped(RVA addr)
{
    CORE_LOCK();
    return rz_io_map_get(core->io, addr);
}

QList<SearchDescription> ClutterCore::getAllSearchCommand(const QString &searchFor, SearchKind kind,
                                                         const QString &in)
{
    CORE_LOCK();
...
static UniquePtrC<RzSearchOpt, &rz_search_opt_free> clutterSetupSearchOptions(RzCore *core)
{
...
class ClutterSearchLock
{
public:
    ClutterSearchLock(RzCore *core) : core(core)
    {
...
static QString clutterGetSearchHitData(RzCore *core, SearchKind kind, RzSearchHit *hit)
{
    QString data = "";
...
static QString clutterValueAsHex(const QString &strVal, bool bigEndian, size_t size)
{
...
QList<SearchDescription> ClutterCore::getAllSearch(QString searchFor, SearchKind kind,
                                                  const QString &in)
{
    ...
    if (core->in_search) {
        ...
        RZ_LOG_ERROR("clutter: recursive search detected, search aborted.\n");
        return {};
    }

    // this takes ownership of the search lock
    const ClutterSearchLock searchLock(core);
    ...
    auto userOpts = clutterSetupSearchOptions(core);
    if (!userOpts) {
        return searchRef;
    }
    ...
    case SearchKind::Value32BE: {
        searchFor = clutterValueAsHex(searchFor, true, 32);
        RzSearchBytesPattern *pattern =
    ...
    case SearchKind::Value32LE: {
        searchFor = clutterValueAsHex(searchFor, false, 32);
        RzSearchBytesPattern *pattern =
    ...
    case SearchKind::Value64BE: {
        searchFor = clutterValueAsHex(searchFor, true, 64);
        RzSearchBytesPattern *pattern =
    ...
    case SearchKind::Value64LE: {
        searchFor = clutterValueAsHex(searchFor, false, 64);
        RzSearchBytesPattern *pattern =
    ...
    RzListIter *it;
    RzSearchHit *hit;
    ClutterRzListForeach (hits, it, RzSearchHit, hit) {
        SearchDescription exp;
        const QString detail = fromOwnedCharPtr(rz_search_hit_detail_as_string(hit));

        exp.offset = hit->address;
        exp.size = hit->size;
        exp.data = clutterGetSearchHitData(core, kind, hit).trimmed();
...
QList<XrefDescription> ClutterCore::collectXRefsForVariable(const QString &variableName, RVA offset,
                                                           int accessTypeMask, bool stopAtFirst)
{
    ...
    for (const auto &v : ClutterPVector<RzAnalysisVar>(&fcn->vars)) {
        if (variableName != v->name) {
            continue;
        }
        RzAnalysisVarAccess *acc;
        ClutterRzVectorForeach(&v->accesses, acc, RzAnalysisVarAccess)
        {
...
QList<XrefDescription> ClutterCore::getXRefsForVariable(const QString &variableName, bool findWrites,
                                                       RVA offset)
{
    const ut8 mask =
...
XrefDescription ClutterCore::getFirstXRefForVariable(const QString &variableName, RVA offset)
{
...
QList<XrefDescription> ClutterCore::getXRefs(RVA addr, bool to, bool whole_function,
                                            const QString &filterType)
{
    ...
    RzListIter *it;
    RzAnalysisXRef *xref;
    ClutterRzListForeach (xrefs, it, RzAnalysisXRef, xref) {
        XrefDescription xd;
        xd.from = xref->from;
        xd.to = xref->to;
...
QString ClutterCore::getXRefCommentAt(RVA offset)
{
    CORE_LOCK();
    QString commentStr;
...
void ClutterCore::addGlobalVariable(RVA offset, QString name, const QString &typ)
{
...
void ClutterCore::modifyGlobalVariable(RVA offset, QString name, const QString &typ)
{
    name = sanitizeStringForCommand(name);
    CORE_LOCK();
...
void ClutterCore::delGlobalVariable(QString name)
{
    name = sanitizeStringForCommand(name);
    CORE_LOCK();
...
void ClutterCore::delGlobalVariable(RVA offset)
{
    CORE_LOCK();
...
QString ClutterCore::getGlobalVariableType(QString name)
{
    name = sanitizeStringForCommand(name);
    CORE_LOCK();
...
QString ClutterCore::getGlobalVariableType(RVA offset)
{
    CORE_LOCK();
...
void ClutterCore::addFlag(RVA offset, QString name, RVA size)
{
    name = sanitizeStringForCommand(name);
    CORE_LOCK();
...
QString ClutterCore::listFlagsAsStringAt(RVA addr)
{
    CORE_LOCK();
...
QString ClutterCore::nearestFlag(RVA offset, RVA *flagOffsetOut)
{
    CORE_LOCK();
...
void ClutterCore::addMark(RVA from, RVA to, const QString &name, const QString &comment,
                         QColor color)
{
    CORE_LOCK();
...
void ClutterCore::delMark(const QString &name)
{
    CORE_LOCK();
...
QList<MarkDescription> ClutterCore::convertMarks(RzList *marks)
{
    QList<MarkDescription> markList;

    RzListIter *it;
    RzMarkItem *mark;
    ClutterRzListForeach (marks, it, RzMarkItem, mark) {
...
QList<MarkDescription> ClutterCore::getMarks()
{
    CORE_LOCK();
    return convertMarks(rz_mark_all_list(core->marks));
}

QList<MarkDescription> ClutterCore::getMarksAt(RVA addr)
{
    CORE_LOCK();
    return convertMarks(rz_mark_get_all_off(core->marks, addr));
}

QColor ClutterCore::getBlendedMarksColorAt(RVA addr)
{
...
void ClutterCore::handleREvent(int type, void *data)
{
    switch (type) {
...
void ClutterCore::triggerFlagsChanged()
{
    emit flagsChanged();
}

void ClutterCore::triggerVarsChanged()
{
    emit varsChanged();
}

void ClutterCore::triggerFunctionRenamed(const RVA offset, const QString &newName)
{
    emit functionRenamed(offset, newName);
}

void ClutterCore::loadPDB(const QString &file)
{
    CORE_LOCK();
    rz_core_bin_pdb_load(core, file.toUtf8().constData());
}

void ClutterCore::applyDwarf()
{
    CORE_LOCK();
    rz_core_bin_apply_dwarf(core, rz_bin_cur(core->bin));
}

QList<DisassemblyLine> ClutterCore::disassembleLines(RVA offset, int lines)
{
    CORE_LOCK();
    auto vec = fromOwned(
    ...
    for (const auto &t : ClutterPVector<RzAnalysisDisasmText>(vec.get())) {
...
QString ClutterCore::hexdump(RVA address, int size, HexdumpFormats format)
{
    CORE_LOCK();
    char *res = nullptr;
    switch (format) {
...
QByteArray ClutterCore::hexStringToBytes(const QString &hex)
{
...
QString ClutterCore::bytesToHexString(const QByteArray &bytes)
{
    return QString::fromUtf8(bytes.toHex());
}

void ClutterCore::loadScript(const QString &scriptname)
{
    {
        CORE_LOCK();
...
QString ClutterCore::getRizinVersionReadable(const char *program)
{
    return fromOwnedCharPtr(rz_version_str(rzCore->sys_path, program));
}

QString ClutterCore::getVersionInformation()
{
    int i;
    QString versionInfo;
    const struct Vcs
...
QStringList ClutterCore::getColorThemes()
{
    QStringList r;
    CORE_LOCK();
    ...
    for (const auto &th : ClutterPVector<char>(themesList)) {
...
QHash<QString, QColor> ClutterCore::getTheme()
{
...
QStringList ClutterCore::getThemeKeys()
{
...
bool ClutterCore::setColor(const QString &key, const QString &color)
{
...
QString ClutterCore::getColorNameFromOp(ut32 opType)
{
    CORE_LOCK();
    return rz_print_color_op_type(core->print, opType);
}

QString ClutterCore::ansiEscapeToHtml(const QString &text)
{
    int len;
    QString r = text;
    r.replace("\t", "        ");
...
BasicBlockHighlighter *ClutterCore::getBBHighlighter()
{
    return bbHighlighter;
}

BasicInstructionHighlighter *ClutterCore::getBIHighlighter()
{
    return &biHighlighter;
}

void ClutterCore::setIOCache(bool enabled)
{
    if (enabled) {
...
bool ClutterCore::isIOCacheEnabled() const
{
    return iocache;
}

void ClutterCore::commitWriteCache()
{
    CORE_LOCK();
...
void ClutterCore::resetWriteCache()
{
    CORE_LOCK();
...
void ClutterCore::setWriteMode(bool enabled)
{
...
bool ClutterCore::isWriteModeEnabled()
{
    CORE_LOCK();
    RzListIter *it;
    RzCoreFile *cf;
    ClutterRzListForeach (core->files, it, RzCoreFile, cf) {
...
bool ClutterCore::hasUncommitedChanges()
{

    CORE_LOCK();
    for (auto c : ClutterPVector<RzIOCache>(&core->io->cache)) {
        if (!c->written) {
            return false;
        }
    }
...
QStringList ClutterCore::getDisassemblyPreview(RVA address, int num_of_lines)
{
...
QString ClutterCore::getHexdumpPreview(RVA address, int size)
{
    // temporarily simplify the disasm output to get it colorful and simple to read
    TempConfig tempConfig;
    tempConfig.set("scr.color", COLOR_MODE_16M)
...
QByteArray ClutterCore::ioRead(RVA addr, int len)
{
    CORE_LOCK();
...
QStringList ClutterCore::getConfigVariableSpaces(const QString &key)
{
    CORE_LOCK();
    ...
    QStringList stringList;
    for (const auto &x : ClutterRzList<char>(list)) {
        stringList << x;
    }
    rz_list_free(list);
    return stringList;
...
void ClutterCore::resetConfig()
{
    CORE_LOCK();
    rz_core_config_init(core);
}

char *ClutterCore::getTextualGraphAt(RzCoreGraphType type, RzCoreGraphFormat format, RVA address)
{
    CORE_LOCK();
...
void ClutterCore::writeGraphvizGraphToFile(const QString &path, const QString &format,
...
void ClutterCore::showTypeInTypesWidget(const QString &typeName)
{
    emit showTypeRequested(typeName);
}

void ClutterCore::renameType(const QString &from, const QString &to)
{
    CORE_LOCK();
...
void ClutterCore::setTypeClass(const QString &type, const QString &typeClass)
{
    CORE_LOCK();
    const RzTypeDB *typedb = rz_analysis_get_type_db(core->analysis);
