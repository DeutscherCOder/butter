#ifndef BUTTER_JSON_H
#define BUTTER_JSON_H
...
class ButterJsonOwner;

/**
 * @brief Butter wrapper for handling Json
 */
class BUTTER_EXPORT ButterJson
{
public:
    class Iterator
    {
    public:
        Iterator(const RzJson *value, std::shared_ptr<ButterJsonOwner> owner)
            : value(value), owner(std::move(owner))
        {
        }

        ButterJson operator*() const { return ButterJson(value, owner); }
    ...
    private:
        const RzJson *value;
        std::shared_ptr<ButterJsonOwner> owner;
    };

    ButterJson() : value(nullptr) {}

    ButterJson(const RzJson *value, std::shared_ptr<ButterJsonOwner> owner)
        : value(value), owner(std::move(owner))
    {
    }

    ButterJson first() const
    {
        return ButterJson(hasChildren() ? value->children.first : nullptr, owner);
    }

    ButterJson last() const;

    ButterJson operator[](const QString &key) const
    {
        QByteArray utf8 = key.toUtf8();
        return (*this)[utf8.data()];
    }

    ButterJson operator[](const char *key) const
    {
        return ButterJson(
...
class BUTTER_EXPORT ButterJsonOwner
{
public:
    ButterJsonOwner(RzJson *value, char *text) : value(value), text(text) {}

    virtual ~ButterJsonOwner()
    {
        rz_json_free(value);
        rz_mem_free(text);
    }
