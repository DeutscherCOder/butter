...
static ButterCore *uniqueInstance;
...
RzCoreLocked::RzCoreLocked(ButterCore *core) : core(core)
{
    core->coreMutex.lock();
...
static void butterREventCallback(RzEvent *, int type, void *user, void *data)
{
    auto core = reinterpret_cast<ButterCore *>(user);
    core->handleREvent(type, data);
}

ButterCore::ButterCore(QObject *parent)
    : QObject(parent)
...
{
    if (uniqueInstance) {
        throw std::logic_error("Only one instance of ButterCore must exist");
    }
    uniqueInstance = this;
}

ButterCore *ButterCore::instance()
{
    return uniqueInstance;
}

void ButterCore::initialize(bool loadPlugins)
{
    ...
    rz_event_hook(rzCore->ev, RZ_EVENT_ALL, butterREventCallback, this);
    ...
    // Otherwise Rizin may ask the user for input and Butter would freeze
...
RzCoreLocked ButterCore::lock()
{
    return RzCoreLocked(this);
}

RzCoreLocked ButterCore::core()
{
    return lock();
}

QDir ButterCore::getButterRCDefaultDirectory() const
{
    return QStandardPaths::writableLocation(QStandardPaths::AppConfigLocation);
}

QVector<QString> ButterCore::getButterRCFilePaths() const
{
    QVector<QString> result;
    result.push_back(QFileInfo(QDir::home(), ".butterrc").absoluteFilePath());
    const QStringList locations =
            QStandardPaths::standardLocations(QStandardPaths::AppConfigLocation);
    for (auto &location : locations) {
        result.push_back(QFileInfo(QDir(location), ".butterrc").absoluteFilePath());
    }
    result.push_back(QFileInfo(getButterRCDefaultDirectory(), "rc")
                             .absoluteFilePath()); // File in config editor is from this path
    return result;
}

void ButterCore::loadButterRC()
{
    CORE_LOCK();
    const auto result = getButterRCFilePaths();
    for (auto &butterRCFilePath : result) {
        auto butterRCFileInfo = QFileInfo(butterRCFilePath);
        if (!butterRCFileInfo.exists() || !butterRCFileInfo.isFile()) {
            continue;
        }
        qInfo() << tr("Loading initialization file from ") << butterRCFilePath;
        rz_core_cmd_file(core, butterRCFilePath.toUtf8().constData());
        rz_cons_flush();
    }
}

void ButterCore::loadDefaultButterRC()
{
    CORE_LOCK();
    auto butterRCFilePath = QFileInfo(getButterRCDefaultDirectory(), "rc").absoluteFilePath();
    const auto butterRCFileInfo = QFileInfo(butterRCFilePath);
    if (!butterRCFileInfo.exists() || !butterRCFileInfo.isFile()) {
        return;
    }
    qInfo() << tr("Loading initialization file from ") << butterRCFilePath;
...
QList<QString> ButterCore::sdbList(const QString &path)
{
    CORE_LOCK();
    QList<QString> list = QList<QString>();
    const Sdb *root = sdb_ns_path(core->sdb, path.toUtf8().constData(), 0);
    if (root && root->ns) {
        for (const auto &nsi : ButterRzList<SdbNs>(root->ns)) {
            list << nsi->name;
        }
    }
    return list;
...
QList<QString> ButterCore::sdbListKeys(const QString &path)
{
    CORE_LOCK();
    QList<QString> list = QList<QString>();
...
QString ButterCore::sdbGet(const QString &path, const QString &key)
{
    CORE_LOCK();
...
bool ButterCore::sdbSet(const QString &path, const QString &key, const QString &val)
{
    CORE_LOCK();
...
QString ButterCore::sanitizeStringForCommand(QString s)
{
    static const QRegularExpression regexp(";|@");
    return s.replace(regexp, QStringLiteral("_"));
}

QString ButterCore::cmd(const char *str)
{
    CORE_LOCK();
...
QString ButterCore::getFunctionExecOut(const std::function<bool(RzCore *)> &fcn, const RVA addr)
{
    CORE_LOCK();
...
bool ButterCore::isRedirectableDebugee() const
{
    ...
    RzListIter *it;
    RzIODesc *desc;
    ButterRzListForeach (descs, it, RzIODesc, desc) {
...
bool ButterCore::isDebugTaskInProgress()
{
    if (debugTask) {
        return true;
    }
...
void ButterCore::setProfileDirectives(const QString &directives)
{
...
void ButterCore::setRegisterProfile(const QString &profile)
{
    CORE_LOCK();
...
QString ButterCore::convertGdbProfile(const QString &profilePath)
{
    return QString::fromUtf8(rz_reg_parse_gdb_profile(profilePath.toUtf8().constData()));
}

QString ButterCore::getRegisterProfile()
{
    CORE_LOCK();
...
bool ButterCore::asyncTask(std::function<void *(RzCore *)> fcn, std::shared_ptr<RizinTask> &task)
{
    if (task) {
        return false;
    }
...
void ButterCore::functionTask(std::function<void *(RzCore *)> fcn)
{
...
QString ButterCore::cmdRawAt(const char *cmd, RVA address)
{
    QString res;
...
QString ButterCore::cmdRaw(const char *cmd)
{
    const QString res;
    CORE_LOCK();
...
ButterJson ButterCore::cmdj(const char *str)
{
    char *res;
    {
        CORE_LOCK();
...
QString ButterCore::cmdTask(const QString &str)
{
...
ButterJson ButterCore::parseJson(const char *name, char *res, const char *cmd)
{
    ...
    return ButterJson(doc, std::make_shared<ButterJsonOwner>(doc, res));
}

QStringList ButterCore::autocomplete(const QString &cmd, RzLinePromptType promptType)
{
    RzLineBuffer buf;
...
bool ButterCore::loadFile(const QString &path, ut64 baddr, ut64 mapaddr, int perms, int va,
                          bool loadbin, const QString &forceBinPlugin)
{
    ...
    if (perms & RZ_PERM_W) {
        ...
        for (auto map : ButterPVector<RzIOMap>(maps)) {
            map->perm |= RZ_PERM_W;
        }
    }
...
bool ButterCore::tryFile(const QString &path, bool rw)
{
    if (path.isEmpty()) {
...
bool ButterCore::mapFile(const QString &path, RVA mapaddr)
{
    CORE_LOCK();
...
void ButterCore::renameFunction(const RVA offset, const QString &newName)
{
    CORE_LOCK();
...
void ButterCore::delFunction(RVA addr)
{
    CORE_LOCK();
...
void ButterCore::renameFlag(const QString &old_name, const QString &new_name)
{
    CORE_LOCK();
...
void ButterCore::renameFunctionVariable(const QString &newName, const QString &oldName,
                                        RVA functionAddress)
{
    CORE_LOCK();
...
void ButterCore::delFlag(RVA addr)
{
    CORE_LOCK();
...
void ButterCore::delFlag(const QString &name)
{
    CORE_LOCK();
...
ButterRzIter<RzCoreDecodedBytes> ButterCore::getRzCoreDecodedBytesSingle(RVA addr)
{
    CORE_LOCK();
    ut8 buf[128];
    ...
    auto result = ButterRzIter<RzCoreDecodedBytes>(
            rz_core_analysis_bytes(core, addr, buf, sizeof(buf), 1));
    return result;
}

QString ButterCore::getInstructionBytes(RVA addr)
{
    auto cdb = getRzCoreDecodedBytesSingle(addr);
    return cdb ? cdb->bytes : "";
}

QString ButterCore::getInstructionOpcode(RVA addr)
{
    auto cdb = getRzCoreDecodedBytesSingle(addr);
    return cdb ? cdb->opcode : "";
}

void ButterCore::editInstruction(RVA addr, const QString &inst, bool fillWithNops)
{
    CORE_LOCK();
    if (fillWithNops) {
...
void ButterCore::nopInstruction(RVA addr)
{
    CORE_LOCK();
    {
...
void ButterCore::jmpReverse(RVA addr)
{
    CORE_LOCK();
    {
        auto seek = seekTemp(addr);
...
void ButterCore::editBytes(RVA addr, const QString &bytes)
{
    CORE_LOCK();
...
void ButterCore::editBytesEndian(RVA addr, const QString &bytes)
{
    CORE_LOCK();
...
void ButterCore::setToCode(RVA addr)
{
    CORE_LOCK();
...
void ButterCore::setAsString(RVA addr, int size, StringTypeFormats type)
{
    if (RVA_INVALID == addr) {
        return;
    }
...
void ButterCore::removeString(RVA addr)
{
    CORE_LOCK();
...
QString ButterCore::getString(RVA addr)
{
    CORE_LOCK();
...
QString ButterCore::getString(RVA addr, uint64_t len, RzStrEnc encoding, bool escape_nl)
{
    CORE_LOCK();
...
QString ButterCore::getMetaString(RVA addr)
{
    CORE_LOCK();
    return rz_meta_get_string(core->analysis, RZ_META_TYPE_STRING, addr);
}

void ButterCore::setToData(RVA addr, int size, int repeat)
{
    if (size <= 0 || repeat <= 0) {
        return;
    }
...
int ButterCore::sizeofDataMeta(RVA addr)
{
    ut64 size = 0;
    CORE_LOCK();
...
void ButterCore::setComment(RVA addr, const QString &cmt)
{
    CORE_LOCK();
...
void ButterCore::delComment(RVA addr)
{
    CORE_LOCK();
...
QString ButterCore::getCommentAt(RVA addr)
{
    CORE_LOCK();
    return rz_meta_get_string(core->analysis, RZ_META_TYPE_COMMENT, addr);
}

void ButterCore::setImmediateBase(const QString &rzBaseName, RVA offset)
{
...
void ButterCore::setCurrentBits(int bits, RVA offset)
{
    if (offset == RVA_INVALID) {
        offset = getOffset();
    }
...
void ButterCore::applyStructureOffset(const QString &structureOffset, RVA offset)
{
    if (offset == RVA_INVALID) {
        offset = getOffset();
    }
...
void ButterCore::seekSilent(ut64 offset)
{
    CORE_LOCK();
    if (offset == RVA_INVALID) {
        return;
...
void ButterCore::seek(ut64 offset)
{
...
void ButterCore::showMemoryWidget()
{
    emit showMemoryWidgetRequested();
}

void ButterCore::seekAndShow(ut64 offset)
{
    seek(offset);
    emit showAddressRequested(offset);
}

void ButterCore::seekAndShow(const QString &offset)
{
    seek(offset);
    emit showAddressRequested(math(offset));
}

void ButterCore::seek(const QString &thing)
{
    CORE_LOCK();
...
void ButterCore::updateSeek(SeekHistoryType type)
{
    emit seekChanged(getOffset(), type);
}

RVA ButterCore::prevOpAddr(RVA startAddr, int count)
{
    CORE_LOCK();
    return rz_core_prevop_addr_force(core, startAddr, count);
}

RVA ButterCore::nextOpAddr(RVA startAddr, int count)
{
    CORE_LOCK();
...
RVA ButterCore::getOffset()
{
    return rzCore->offset;
}

void ButterCore::applySignature(const QString &filepath)
{
    CORE_LOCK();
    int oldCnt, newCnt;
...
void ButterCore::createSignature(const QString &filepath)
{
    CORE_LOCK();
    ut32 nModules = 0;
...
bool ButterCore::isValidInputNumValue(const QString &expression)
{
    CORE_LOCK();
    return rz_is_valid_input_num_value(core ? core->num : nullptr, expression.toUtf8().constData());
}

ut64 ButterCore::math(const QString &expr)
{
    CORE_LOCK();
    return rz_num_math(core ? core->num : nullptr, expr.toUtf8().constData());
}

ut64 ButterCore::num(const QString &expr)
{
    CORE_LOCK();
    return rz_num_get(core ? core->num : nullptr, expr.toUtf8().constData());
}

QString ButterCore::itoa(ut64 num, int rdx)
{
    return QString::number(num, rdx);
}

void ButterCore::setConfig(const char *k, const char *v)
{
    CORE_LOCK();
    rz_config_set(core->config, k, v);
}

void ButterCore::setConfig(const QString &k, const char *v)
{
    CORE_LOCK();
    rz_config_set(core->config, k.toUtf8().constData(), v);
}

void ButterCore::setConfig(const char *k, const QString &v)
{
    CORE_LOCK();
    rz_config_set(core->config, k, v.toUtf8().constData());
}

AddressTypeHint ButterCore::getAddressType(RVA addr)
{
...
void ButterCore::setConfig(const char *k, int v)
{
    CORE_LOCK();
    rz_config_set_i(core->config, k, static_cast<ut64>(v));
}

void ButterCore::setConfig(const char *k, ut64 v)
{
    CORE_LOCK();
    rz_config_set_integer(core->config, k, v);
}

void ButterCore::setConfig(const char *k, bool v)
{
    CORE_LOCK();
    rz_config_set_i(core->config, k, v ? 1 : 0);
}

void ButterCore::setConfig(const char *k, const RzInterval &itv)
{
    CORE_LOCK();
    rz_config_set_interval(core->config, k, itv);
}

void ButterCore::setConfig(const char *k, const QStringList &list)
{
    CORE_LOCK();
...
int ButterCore::getConfigi(const char *k)
{
    CORE_LOCK();
    return static_cast<int>(rz_config_get_i(core->config, k));
}

ut64 ButterCore::getConfigut64(const char *k)
{
    CORE_LOCK();
    return rz_config_get_i(core->config, k);
}

bool ButterCore::getConfigb(const char *k)
{
    CORE_LOCK();
    return rz_config_get_i(core->config, k) != 0;
}

RzInterval ButterCore::getConfigItv(const char *k)
{
    CORE_LOCK();
    return rz_config_get_interval(core->config, k);
}

QSet<QString> ButterCore::getConfigSet(const char *k)
{
    CORE_LOCK();
...
QString ButterCore::getConfigDescription(const char *k)
{
    CORE_LOCK();
...
void ButterCore::triggerRefreshAll()
{
    emit refreshAll();
}

void ButterCore::triggerAsmOptionsChanged()
{
    emit asmOptionsChanged();
}

void ButterCore::triggerGraphOptionsChanged()
{
    emit graphOptionsChanged();
}

void ButterCore::triggerDebugOptionsChanged()
{
    emit debugOptionsChanged();
}

void ButterCore::triggerAnalysisOptionsChanged()
{
    emit analysisOptionsChanged();
}

void ButterCore::triggerSymbolsOptionsChanged()
{
    emit symbolsOptionsChanged();
}

void ButterCore::message(const QString &msg, bool debug)
{
    if (msg.isEmpty()) {
        return;
    }
...
QString ButterCore::getConfig(const char *k)
{
    CORE_LOCK();
    return { rz_config_get(core->config, k) };
}

QSet<QString> ButterCore::getConfigOptions(const char *k)
{
    CORE_LOCK();
    const RzConfigNode *node = rz_config_node_get(core->config, k);
    ...
    QSet<QString> res;
    for (auto it = ButterRzIter<const char *>(rz_set_s_as_iter(node->options)); it; ++it) {
        res << QString::fromUtf8(*it);
    }
    return res;
}

void ButterCore::setConfig(const char *k, const QVariant &v)
{
...
void ButterCore::setCPU(const QString &arch, const QString &cpu, int bits)
{
    if (arch != nullptr) {
...
void ButterCore::setEndianness(bool big)
{
    setConfig("cfg.bigendian", big);
}

QByteArray ButterCore::assemble(const QString &code)
{
    CORE_LOCK();
...
QString ButterCore::disassemble(const QByteArray &data)
{
    CORE_LOCK();
...
QString ButterCore::disassembleSingleInstruction(RVA addr)
{
    auto cdb = getRzCoreDecodedBytesSingle(addr);
    return QString(cdb->disasm).simplified();
}

RzAnalysisFunction *ButterCore::functionIn(ut64 addr)
{
    CORE_LOCK();
...
RzAnalysisFunction *ButterCore::functionAt(ut64 addr)
{
    CORE_LOCK();
    return rz_analysis_get_function_at(core->analysis, addr);
}

RVA ButterCore::getFunctionStart(RVA addr)
{
    const auto core = Core()->lock();
...
RVA ButterCore::getFunctionEnd(RVA addr)
{
    const auto core = Core()->lock();
    const RzAnalysisFunction *fcn = Core()->functionIn(addr);
    return fcn ? fcn->addr : RVA_INVALID;
...
RVA ButterCore::getLastFunctionInstruction(RVA addr)
{
    const auto core = Core()->lock();
    const RzAnalysisFunction *fcn = Core()->functionIn(addr);
    if (!fcn) {
...
QString ButterCore::flagAt(RVA addr, bool getClosestFlag)
{
    CORE_LOCK();
...
void ButterCore::createFunctionAt(RVA addr)
{
    createFunctionAt(addr, "");
}

void ButterCore::createFunctionAt(RVA addr, QString name)
{
...
RVA ButterCore::getOffsetJump(RVA addr)
{
    auto cdb = getRzCoreDecodedBytesSingle(addr);
    return cdb ? cdb->an_op.jump : RVA_INVALID;
}

QList<Decompiler *> ButterCore::getDecompilers()
{
    return decompilers;
}

Decompiler *ButterCore::getDecompilerById(const QString &id)
{
...
bool ButterCore::registerDecompiler(Decompiler *decompiler)
{
...
ButterJson ButterCore::getSignatureInfo()
{
    CORE_LOCK();
...
bool ButterCore::existsFileInfo()
{
    CORE_LOCK();
...
RefDescription ButterCore::formatRefDesc(const std::shared_ptr<AddrRefs> &refItem)
{
    RefDescription desc;
...
QList<RegisterRef> ButterCore::getRegisterRefs(int depth)
{
    ...
    RzListIter *it;
    RzRegItem *ri;
    ButterRzListForeach (ritems, it, RzRegItem, ri) {
        RegisterRef reg;
...
QList<AddrRefs> ButterCore::getStack(int size, int depth)
{
...
AddrRefs ButterCore::getAddrRefs(RVA addr, int depth)
{
    AddrRefs refs;
...
QVector<Chunk> ButterCore::getHeapChunks(RVA arena_addr)
{
    CORE_LOCK();
    ...
    ButterRzListForeach (chunks, iter, RzHeapChunkListItem, data) {
        Chunk chunk;
...
int ButterCore::getArchBits()
{
    CORE_LOCK();
    return core->dbg->bits;
}

QVector<Arena> ButterCore::getArenas()
{
    CORE_LOCK();
    ...
    ButterRzListForeach (arenas, iter, RzArenaListItem, data) {
        Arena arena;
...
RzHeapChunkSimple *ButterCore::getHeapChunk(ut64 addr)
{
    CORE_LOCK();
    return rz_heap_chunk(core, addr);
}

QVector<RzHeapBin *> ButterCore::getHeapBins(ut64 arena_addr)
{
    CORE_LOCK();
    ...
    RzListIter *iter;
    RzHeapBin *bin;
    ButterRzListForeach (tcacheBins, iter, RzHeapBin, bin) {
        if (!bin) {
            continue;
        }
        if (!rz_list_length(bin->chunks)) {
...
bool ButterCore::writeHeapChunk(RzHeapChunkSimple *chunk_simple)
{
    CORE_LOCK();
    return rz_heap_write_chunk(core, chunk_simple);
}

QList<VariableDescription> ButterCore::getVariables(RVA at)
{
    ...
    for (auto var : ButterPVector<RzAnalysisVar>(&fcn->vars)) {
...
QList<GlobalDescription> ButterCore::getAllGlobals()
{
    CORE_LOCK();
    RzListIter *it;
    ...
    if (core && core->analysis) {
        ...
        if (typedb) {
            ...
            ButterRzListForeach (globals, it, RzAnalysisVarGlobal, glob) {
...
QVector<RegisterRefValueDescription> ButterCore::getRegisterRefValues()
{
...
RegisterRefValueDescription ButterCore::getRegisterRefValue(const QString &regName)
{
    RegisterRefValueDescription desc;
    CORE_LOCK();
...
QString ButterCore::getRegisterName(const QString &registerRole)
{
    if (!currentlyDebugging) {
        return "";
    }
...
RVA ButterCore::getProgramCounterValue()
{
    if (currentlyDebugging) {
        CORE_LOCK();
...
void ButterCore::setRegister(const QString &regName, const QString &regValue)
{
    if (!currentlyDebugging) {
        return;
    }
...
void ButterCore::setCurrentDebugThread(int tid)
{
    if (!asyncTask(
                [=](RzCore *core) {
...
void ButterCore::setCurrentDebugProcess(int pid)
{
    if (!currentlyDebugging
        || !asyncTask(
                [=](RzCore *core) {
...
void ButterCore::startEmulation()
{
    if (!currentlyDebugging) {
        offsetPriorDebugging = getOffset();
    }
...
void ButterCore::attachRemote(const QString &uri)
{
    if (!currentlyDebugging) {
        offsetPriorDebugging = getOffset();
    }
...
void ButterCore::attachDebug(int pid)
{
    if (!currentlyDebugging) {
        offsetPriorDebugging = getOffset();
    }
...
void ButterCore::suspendDebug()
{
    debugTask->breakTask();
    debugTask->joinTask();
}

void ButterCore::stopDebug()
{
    if (!currentlyDebugging) {
        return;
    }
...
void ButterCore::syncAndSeekProgramCounter()
{
    seekAndShow(getProgramCounterValue());
    emit registersChanged();
}

void ButterCore::continueDebug()
{
    if (!currentlyDebugging) {
        return;
    }
...
void ButterCore::continueBackDebug()
{
    if (!currentlyDebugging) {
        return;
    }
...
void ButterCore::continueUntilDebug(ut64 offset)
{
    if (!currentlyDebugging) {
        return;
    }
...
void ButterCore::continueUntilCall()
{
    if (!currentlyDebugging) {
        return;
    }
...
void ButterCore::continueUntilSyscall()
{
    if (!currentlyDebugging) {
        return;
    }
...
QStringList ButterCore::getDebugPlugins()
{
    QStringList plugins;
    CORE_LOCK();
    ButterHtSP<RzDebugPlugin>(core->dbg->plugins)
...
QString ButterCore::getActiveDebugPlugin()
{
    return getConfig("dbg.backend");
}

void ButterCore::setDebugPlugin(const QString &plugin)
{
    setConfig("dbg.backend", plugin);
}

void ButterCore::startTraceSession()
{
...
void ButterCore::stopTraceSession()
{
    if (!currentlyDebugging || !currentlyTracing) {
        return;
    }
...
void ButterCore::toggleBreakpoint(RVA addr)
{
    CORE_LOCK();
...
void ButterCore::addBreakpoint(const BreakpointDescription &config)
{
    CORE_LOCK();
...
void ButterCore::updateBreakpoint(int index, const BreakpointDescription &config)
{
    CORE_LOCK();
...
void ButterCore::delBreakpoint(RVA addr)
{
    CORE_LOCK();
...
void ButterCore::delAllBreakpoints()
{
    CORE_LOCK();
...
void ButterCore::enableBreakpoint(RVA addr)
{
    CORE_LOCK();
...
void ButterCore::disableBreakpoint(RVA addr)
{
    CORE_LOCK();
...
void ButterCore::setBreakpointTrace(int index, bool enabled)
{
    CORE_LOCK();
...
int ButterCore::breakpointIndexAt(RVA addr)
{
    CORE_LOCK();
    return rz_bp_get_index_at(core->dbg->bp, addr);
}

BreakpointDescription ButterCore::getBreakpointAt(RVA addr)
{
    CORE_LOCK();
...
QList<BreakpointDescription> ButterCore::getBreakpoints()
{
    CORE_LOCK();
...
QList<RVA> ButterCore::getBreakpointsAddresses()
{
...
QList<RVA> ButterCore::getBreakpointsInFunction(RVA funcAddr)
{
...
bool ButterCore::isBreakpoint(const QList<RVA> &breakpoints, RVA addr)
{
    return breakpoints.contains(addr);
}

QList<ThreadDescription> ButterCore::getProcessThreads(int pid)
{
    CORE_LOCK();
    auto dbg = core->dbg;
    ...
    ButterRzListForeach (list, iter, RzDebugPid, p) {
...
QList<ProcessDescription> ButterCore::getProcesses(int pid)
{
    CORE_LOCK();
    auto dbg = core->dbg;
    if (!dbg || !dbg->cur || !dbg->cur->threads) {
...
QList<MemoryMapDescription> ButterCore::getMemoryMap()
{
    CORE_LOCK();
    ...
    RzListIter *it;
    RzDebugMap *map;
    ButterRzListForeach (list0, it, RzDebugMap, map) {
...
void ButterCore::setGraphEmpty(bool empty)
{
    emptyGraph = empty;
}

bool ButterCore::isGraphEmpty() const
{
    return emptyGraph;
}

void ButterCore::getRegs()
{
    const auto core = Core()->lock();
    this->regs = {};
    ...
    for (const auto &r : ButterRzList<RzRegItem>(rs)) {
        this->regs.push_back(r->name);
    }
}

void ButterCore::setSettings()
{
    setConfig("scr.interactive", false);
...
QList<RVA> ButterCore::getSeekHistory()
{
    CORE_LOCK();
    QList<RVA> ret;
    RzListIter *it;
...
QStringList ButterCore::getAsmPluginNames()
{
    CORE_LOCK();
    QStringList ret;
    ButterHtSP<RzAsmPlugin>(rz_asm_get_plugins(core->rasm))
...
QStringList ButterCore::getAnalysisPluginNames()
{
    CORE_LOCK();
    QStringList ret;
    ButterHtSP<RzAnalysisPlugin>(rz_analysis_get_plugins(core->analysis))
...
QList<RzBinPluginDescription> ButterCore::getBinPluginDescriptions(bool bin, bool xtr)
{
    CORE_LOCK();
    QList<RzBinPluginDescription> ret;
    if (bin) {
        ButterHtSP<RzBinPlugin>(core->bin->plugins)
    ...
    if (xtr) {
        ButterHtSP<RzBinXtrPlugin>(core->bin->binxtrs)
...
QList<RzIOPluginDescription> ButterCore::getRIOPluginDescriptions()
{
    CORE_LOCK();
    QList<RzIOPluginDescription> ret;
    ButterHtSP<RzIOPlugin>(core->io->plugins)
...
QList<RzCorePluginDescription> ButterCore::getRCorePluginDescriptions()
{
    CORE_LOCK();
    QList<RzCorePluginDescription> ret;
    ButterHtSP<RzCorePlugin>(core->plugins)
...
QList<RzAsmPluginDescription> ButterCore::getRAsmPluginDescriptions()
{
    CORE_LOCK();
...
QList<FunctionDescription> ButterCore::getAllFunctions()
{
    CORE_LOCK();
    ...
    ButterRzListForeach (fcns, iter, RzAnalysisFunction, fcn) {
...
QList<ImportDescription> ButterCore::getAllImports()
{
    CORE_LOCK();
    ...
    for (const auto &import : ButterPVector<RzBinImport>(imports)) {
...
QList<ExportDescription> ButterCore::getAllExports()
{
    CORE_LOCK();
    const RzBinFile *bf = rz_bin_cur(core->bin);
    if (!bf) {
    ...
    for (const auto &symbol : ButterPVector<RzBinSymbol>(symbols)) {
...
QList<SymbolDescription> ButterCore::getAllSymbols()
{
    CORE_LOCK();
    const RzBinFile *bf = rz_bin_cur(core->bin);
    if (!bf) {
    ...
    const RzPVector *symbols = rz_bin_object_get_symbols(bf->o);
    if (symbols) {
        for (const auto &bs : ButterPVector<RzBinSymbol>(symbols)) {
    ...
    if (symbols) {
        ...
        int n = 0;
        for (const auto &entry : ButterPVector<RzBinAddr>(entries)) {
            SymbolDescription symbol;
...
QList<HeaderDescription> ButterCore::getAllHeaders()
{
    CORE_LOCK();
    const RzBinFile *bf = rz_bin_cur(core->bin);
    if (!bf) {
    ...
    for (auto field : ButterPVector<RzBinField>(fields)) {
...
QList<FlirtDescription> ButterCore::getSignaturesDB()
{
    CORE_LOCK();
...
QList<CommentDescription> ButterCore::getAllComments(const QString &filterType)
{
    CORE_LOCK();
...
QList<RelocDescription> ButterCore::getAllRelocs()
{
    CORE_LOCK();
...
QList<StringDescription> ButterCore::getAllStrings(bool raw)
{
    CORE_LOCK();
    ...
    for (const auto &str : ButterPVector<RzBinString>(strings)) {
...
QList<FlagspaceDescription> ButterCore::getAllFlagspaces()
{
    CORE_LOCK();
...
QList<FlagDescription> ButterCore::getAllFlags(const QString &flagspace)
{
    CORE_LOCK();
...
SectionDescription ButterCore::getSectionAtAddress(RVA addr)
{
    CORE_LOCK();
...
QList<SectionDescription> ButterCore::getAllSections()
{
    CORE_LOCK();
    ...
    for (const auto &sect : ButterPVector<RzBinSection>(sects)) {
...
QStringList ButterCore::getSectionList()
{
    CORE_LOCK();
    QStringList ret;
...
QList<SegmentDescription> ButterCore::getAllSegments()
{
    CORE_LOCK();
    const RzBinFile *bf = rz_bin_cur(core->bin);
    if (!bf) {
    ...
    for (const auto &segment : ButterPVector<RzBinSection>(segments)) {
...
QList<EntrypointDescription> ButterCore::getAllEntrypoint()
{
    CORE_LOCK();
    const RzBinFile *bf = rz_bin_cur(core->bin);
    if (!bf) {
...
QList<BinClassDescription> ButterCore::getAllClassesFromBin()
{
    CORE_LOCK();
    const RzBinFile *bf = rz_bin_cur(core->bin);
    if (!bf) {
    ...
    RzBinSymbol *sym;
    RzBinClassField *f;
    for (const auto &c : ButterPVector<RzBinClass>(cs)) {
        BinClassDescription classDescription;
        classDescription.name = c->name;
        classDescription.addr = c->addr;
        ButterRzListForeach (c->methods, iter2, RzBinSymbol, sym) {
        ...
        ButterRzListForeach (c->fields, iter3, RzBinClassField, f) {
...
QList<BinClassDescription> ButterCore::getAllClassesFromFlags()
{
...
QList<QString> ButterCore::getAllAnalysisClasses(bool sorted)
{
    CORE_LOCK();
    QList<QString> ret;
    ...
    for (const auto &kv : ButterPVector<SdbKv>(l.get())) {
        ret.append(QString::fromUtf8(reinterpret_cast<const char *>(kv->base.key)));
    }
    return ret;
}

QList<AnalysisMethodDescription> ButterCore::getAnalysisClassMethods(const QString &cls)
{
    CORE_LOCK();
    ...
    ButterRzVectorForeach(meths, meth, RzAnalysisMethod)
    {
...
QList<AnalysisBaseClassDescription> ButterCore::getAnalysisClassBaseClasses(const QString &cls)
{
    CORE_LOCK();
    ...
    ButterRzVectorForeach(bases, base, RzAnalysisBaseClass)
    {
...
QList<AnalysisVTableDescription> ButterCore::getAnalysisClassVTables(const QString &cls)
{
    CORE_LOCK();
    ...
    ButterRzVectorForeach(vtables, vtable, RzAnalysisVTable)
    {
...
void ButterCore::createNewClass(const QString &cls)
{
    CORE_LOCK();
    rz_analysis_class_create(core->analysis, cls.toUtf8().constData());
}

void ButterCore::renameClass(const QString &oldName, const QString &newName)
{
    CORE_LOCK();
...
void ButterCore::deleteClass(const QString &cls)
{
    CORE_LOCK();
    rz_analysis_class_delete(core->analysis, cls.toUtf8().constData());
}

bool ButterCore::getAnalysisMethod(const QString &cls, const QString &meth,
                                   AnalysisMethodDescription *desc)
{
    CORE_LOCK();
...
void ButterCore::setAnalysisMethod(const QString &className, const AnalysisMethodDescription &meth)
{
    CORE_LOCK();
    RzAnalysisMethod analysisMeth;
...
void ButterCore::renameAnalysisMethod(const QString &className, const QString &oldMethodName,
...
QList<ResourcesDescription> ButterCore::getAllResources()
{
    CORE_LOCK();
    const RzBinFile *bf = rz_bin_cur(core->bin);
    if (!bf) {
    ...
    for (const auto &resource : ButterPVector<RzBinResource>(resources)) {
...
QList<VTableDescription> ButterCore::getAllVTables()
{
    CORE_LOCK();
    ...
    ButterRzListForeach (vtables, iter, RVTableInfo, table) {
        VTableDescription tableDesc;
        tableDesc.addr = table->saddr;
        ButterRzVectorForeach(&table->methods, method, RVTableMethodInfo)
        {
...
QList<BacktraceDescription> ButterCore::getAllBacktraces()
{
    ...
    RzListIter *iter;
    RzBacktrace *bt;
    ButterRzListForeach (list, iter, RzBacktrace, bt) {
...
QList<EvaluableVarDescription> ButterCore::getAllEvaluableVars()
{
    ...
    ButterHtSP<RzConfig>(core->plugin_configs)
...
QList<QString> ButterCore::getAllEvaluableVarSpaces()
{
    CORE_LOCK();
    ...
    RzListIter *iter;
    char *space;
    ButterRzListForeach (spaces, iter, char, space) {
...
QList<TypeDescription> ButterCore::getAllTypes()
{
...
QList<TypeDescription> ButterCore::getBaseType(RzBaseTypeKind kind, const char *category)
{
    CORE_LOCK();
    QList<TypeDescription> types;
    ...
    ButterRzListForeach (ts, iter, RzBaseType, type) {
        TypeDescription exp;
...
QList<TypeDescription> ButterCore::getAllPrimitiveTypes()
{
    return getBaseType(RZ_BASE_TYPE_KIND_ATOMIC, "Primitive");
}

QList<TypeDescription> ButterCore::getAllUnions()
{
    return getBaseType(RZ_BASE_TYPE_KIND_UNION, "Union");
}

QList<TypeDescription> ButterCore::getAllStructs()
{
    return getBaseType(RZ_BASE_TYPE_KIND_STRUCT, "Struct");
}

QList<TypeDescription> ButterCore::getAllEnums()
{
    return getBaseType(RZ_BASE_TYPE_KIND_ENUM, "Enum");
}

QList<TypeDescription> ButterCore::getAllTypedefs()
{
    return getBaseType(RZ_BASE_TYPE_KIND_TYPEDEF, "Typedef");
}

QList<QString> ButterCore::getAllTypeClasses()
{
...
QString ButterCore::getTypeAsC(const QString &name)
{
    CORE_LOCK();
...
bool ButterCore::typeExists(const QString &typeName)
{
    CORE_LOCK();
    return rz_type_exists(rz_analysis_get_type_db(core->analysis), typeName.toUtf8().constData());
}

bool ButterCore::isAddressMapped(RVA addr)
{
    CORE_LOCK();
    return rz_io_map_get(core->io, addr);
}

QList<SearchDescription> ButterCore::getAllSearchCommand(const QString &searchFor, SearchKind kind,
                                                         const QString &in)
{
    CORE_LOCK();
...
static UniquePtrC<RzSearchOpt, &rz_search_opt_free> butterSetupSearchOptions(RzCore *core)
{
...
class ButterSearchLock
{
public:
    ButterSearchLock(RzCore *core) : core(core)
    {
...
static QString butterGetSearchHitData(RzCore *core, SearchKind kind, RzSearchHit *hit)
{
    QString data = "";
...
static QString butterValueAsHex(const QString &strVal, bool bigEndian, size_t size)
{
...
QList<SearchDescription> ButterCore::getAllSearch(QString searchFor, SearchKind kind,
                                                  const QString &in)
{
    ...
    if (core->in_search) {
        ...
        RZ_LOG_ERROR("butter: recursive search detected, search aborted.\n");
        return {};
    }

    // this takes ownership of the search lock
    const ButterSearchLock searchLock(core);
    ...
    auto userOpts = butterSetupSearchOptions(core);
    if (!userOpts) {
        return searchRef;
    }
    ...
    case SearchKind::Value32BE: {
        searchFor = butterValueAsHex(searchFor, true, 32);
        RzSearchBytesPattern *pattern =
    ...
    case SearchKind::Value32LE: {
        searchFor = butterValueAsHex(searchFor, false, 32);
        RzSearchBytesPattern *pattern =
    ...
    case SearchKind::Value64BE: {
        searchFor = butterValueAsHex(searchFor, true, 64);
        RzSearchBytesPattern *pattern =
    ...
    case SearchKind::Value64LE: {
        searchFor = butterValueAsHex(searchFor, false, 64);
        RzSearchBytesPattern *pattern =
    ...
    RzListIter *it;
    RzSearchHit *hit;
    ButterRzListForeach (hits, it, RzSearchHit, hit) {
        SearchDescription exp;
        const QString detail = fromOwnedCharPtr(rz_search_hit_detail_as_string(hit));

        exp.offset = hit->address;
        exp.size = hit->size;
        exp.data = butterGetSearchHitData(core, kind, hit).trimmed();
...
QList<XrefDescription> ButterCore::collectXRefsForVariable(const QString &variableName, RVA offset,
                                                           int accessTypeMask, bool stopAtFirst)
{
    ...
    for (const auto &v : ButterPVector<RzAnalysisVar>(&fcn->vars)) {
        if (variableName != v->name) {
            continue;
        }
        RzAnalysisVarAccess *acc;
        ButterRzVectorForeach(&v->accesses, acc, RzAnalysisVarAccess)
        {
...
QList<XrefDescription> ButterCore::getXRefsForVariable(const QString &variableName, bool findWrites,
                                                       RVA offset)
{
    const ut8 mask =
...
XrefDescription ButterCore::getFirstXRefForVariable(const QString &variableName, RVA offset)
{
...
QList<XrefDescription> ButterCore::getXRefs(RVA addr, bool to, bool whole_function,
                                            const QString &filterType)
{
    ...
    RzListIter *it;
    RzAnalysisXRef *xref;
    ButterRzListForeach (xrefs, it, RzAnalysisXRef, xref) {
        XrefDescription xd;
        xd.from = xref->from;
        xd.to = xref->to;
...
QString ButterCore::getXRefCommentAt(RVA offset)
{
    CORE_LOCK();
    QString commentStr;
...
void ButterCore::addGlobalVariable(RVA offset, QString name, const QString &typ)
{
...
void ButterCore::modifyGlobalVariable(RVA offset, QString name, const QString &typ)
{
    name = sanitizeStringForCommand(name);
    CORE_LOCK();
...
void ButterCore::delGlobalVariable(QString name)
{
    name = sanitizeStringForCommand(name);
    CORE_LOCK();
...
void ButterCore::delGlobalVariable(RVA offset)
{
    CORE_LOCK();
...
QString ButterCore::getGlobalVariableType(QString name)
{
    name = sanitizeStringForCommand(name);
    CORE_LOCK();
...
QString ButterCore::getGlobalVariableType(RVA offset)
{
    CORE_LOCK();
...
void ButterCore::addFlag(RVA offset, QString name, RVA size)
{
    name = sanitizeStringForCommand(name);
    CORE_LOCK();
...
QString ButterCore::listFlagsAsStringAt(RVA addr)
{
    CORE_LOCK();
...
QString ButterCore::nearestFlag(RVA offset, RVA *flagOffsetOut)
{
    CORE_LOCK();
...
void ButterCore::addMark(RVA from, RVA to, const QString &name, const QString &comment,
                         QColor color)
{
    CORE_LOCK();
...
void ButterCore::delMark(const QString &name)
{
    CORE_LOCK();
...
QList<MarkDescription> ButterCore::convertMarks(RzList *marks)
{
    QList<MarkDescription> markList;

    RzListIter *it;
    RzMarkItem *mark;
    ButterRzListForeach (marks, it, RzMarkItem, mark) {
...
QList<MarkDescription> ButterCore::getMarks()
{
    CORE_LOCK();
    return convertMarks(rz_mark_all_list(core->marks));
}

QList<MarkDescription> ButterCore::getMarksAt(RVA addr)
{
    CORE_LOCK();
    return convertMarks(rz_mark_get_all_off(core->marks, addr));
}

QColor ButterCore::getBlendedMarksColorAt(RVA addr)
{
...
void ButterCore::handleREvent(int type, void *data)
{
    switch (type) {
...
void ButterCore::triggerFlagsChanged()
{
    emit flagsChanged();
}

void ButterCore::triggerVarsChanged()
{
    emit varsChanged();
}

void ButterCore::triggerFunctionRenamed(const RVA offset, const QString &newName)
{
    emit functionRenamed(offset, newName);
}

void ButterCore::loadPDB(const QString &file)
{
    CORE_LOCK();
    rz_core_bin_pdb_load(core, file.toUtf8().constData());
}

void ButterCore::applyDwarf()
{
    CORE_LOCK();
    rz_core_bin_apply_dwarf(core, rz_bin_cur(core->bin));
}

QList<DisassemblyLine> ButterCore::disassembleLines(RVA offset, int lines)
{
    CORE_LOCK();
    auto vec = fromOwned(
    ...
    for (const auto &t : ButterPVector<RzAnalysisDisasmText>(vec.get())) {
...
QString ButterCore::hexdump(RVA address, int size, HexdumpFormats format)
{
    CORE_LOCK();
    char *res = nullptr;
    switch (format) {
...
QByteArray ButterCore::hexStringToBytes(const QString &hex)
{
...
QString ButterCore::bytesToHexString(const QByteArray &bytes)
{
    return QString::fromUtf8(bytes.toHex());
}

void ButterCore::loadScript(const QString &scriptname)
{
    {
        CORE_LOCK();
...
QString ButterCore::getRizinVersionReadable(const char *program)
{
    return fromOwnedCharPtr(rz_version_str(rzCore->sys_path, program));
}

QString ButterCore::getVersionInformation()
{
    int i;
    QString versionInfo;
    const struct Vcs
...
QStringList ButterCore::getColorThemes()
{
    QStringList r;
    CORE_LOCK();
    ...
    for (const auto &th : ButterPVector<char>(themesList)) {
...
QHash<QString, QColor> ButterCore::getTheme()
{
...
QStringList ButterCore::getThemeKeys()
{
...
bool ButterCore::setColor(const QString &key, const QString &color)
{
...
QString ButterCore::getColorNameFromOp(ut32 opType)
{
    CORE_LOCK();
    return rz_print_color_op_type(core->print, opType);
}

QString ButterCore::ansiEscapeToHtml(const QString &text)
{
    int len;
    QString r = text;
    r.replace("\t", "        ");
...
BasicBlockHighlighter *ButterCore::getBBHighlighter()
{
    return bbHighlighter;
}

BasicInstructionHighlighter *ButterCore::getBIHighlighter()
{
    return &biHighlighter;
}

void ButterCore::setIOCache(bool enabled)
{
    if (enabled) {
...
bool ButterCore::isIOCacheEnabled() const
{
    return iocache;
}

void ButterCore::commitWriteCache()
{
    CORE_LOCK();
...
void ButterCore::resetWriteCache()
{
    CORE_LOCK();
...
void ButterCore::setWriteMode(bool enabled)
{
...
bool ButterCore::isWriteModeEnabled()
{
    CORE_LOCK();
    RzListIter *it;
    RzCoreFile *cf;
    ButterRzListForeach (core->files, it, RzCoreFile, cf) {
...
bool ButterCore::hasUncommitedChanges()
{

    CORE_LOCK();
    for (auto c : ButterPVector<RzIOCache>(&core->io->cache)) {
        if (!c->written) {
            return false;
        }
    }
...
QStringList ButterCore::getDisassemblyPreview(RVA address, int num_of_lines)
{
...
QString ButterCore::getHexdumpPreview(RVA address, int size)
{
    // temporarily simplify the disasm output to get it colorful and simple to read
    TempConfig tempConfig;
    tempConfig.set("scr.color", COLOR_MODE_16M)
...
QByteArray ButterCore::ioRead(RVA addr, int len)
{
    CORE_LOCK();
...
QStringList ButterCore::getConfigVariableSpaces(const QString &key)
{
    CORE_LOCK();
    ...
    QStringList stringList;
    for (const auto &x : ButterRzList<char>(list)) {
        stringList << x;
    }
    rz_list_free(list);
    return stringList;
...
void ButterCore::resetConfig()
{
    CORE_LOCK();
    rz_core_config_init(core);
}

char *ButterCore::getTextualGraphAt(RzCoreGraphType type, RzCoreGraphFormat format, RVA address)
{
    CORE_LOCK();
...
void ButterCore::writeGraphvizGraphToFile(const QString &path, const QString &format,
...
void ButterCore::showTypeInTypesWidget(const QString &typeName)
{
    emit showTypeRequested(typeName);
}

void ButterCore::renameType(const QString &from, const QString &to)
{
    CORE_LOCK();
...
void ButterCore::setTypeClass(const QString &type, const QString &typeClass)
{
    CORE_LOCK();
    const RzTypeDB *typedb = rz_analysis_get_type_db(core->analysis);
