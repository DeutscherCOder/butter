...
#ifdef RZ_GITTAP
#    define BUTTER_COMPILE_TIME_RZ_VERSION "" RZ_GITTAP
#else
#    define BUTTER_COMPILE_TIME_RZ_VERSION "" RZ_VERSION
#endif

ButterApplication::ButterApplication(int &argc, char **argv) : QApplication(argc, argv)
{
    // Setup application information
    setApplicationVersion(BUTTER_VERSION_FULL);
#ifdef Q_OS_MACOS
    setWindowIcon(QIcon(":/img/butter_macos_simple.svg"));
#else
    setWindowIcon(QIcon(":/img/butter.svg"));
#endif
...
#ifdef Q_OS_WIN
    // Hack to force Butter load internet connection related DLL's
    QSslSocket s;
    s.sslConfiguration();
#endif // Q_OS_WIN
    ...
    const QString localVersion = BUTTER_COMPILE_TIME_RZ_VERSION;
    ...
    if (rzversion != localVersion) {
        ...
        msg.setText(QString(QObject::tr("The version used to compile Butter (%1) does not match "
    ...
    Plugins()->loadPlugins(clOptions.enableButterPlugins);
...
#if defined(Q_OS_MACOS) && defined(BUTTER_ENABLE_PACKAGING)
    {
...
#if defined(Q_OS_WIN) && defined(BUTTER_ENABLE_PACKAGING)
    {
...
void ButterApplication::launchNewInstance(const QStringList &args)
{
    QProcess process(this);
    process.setEnvironment(QProcess::systemEnvironment());
    QStringList allArgs;
    if (!clOptions.enableButterPlugins) {
        allArgs.push_back("--no-butter-plugins");
    }
...
bool ButterApplication::event(QEvent *e)
{
...
bool ButterApplication::loadTranslations()
{
    auto locale = Config()->getCurrLocale();

    bool butterTrLoaded = false;
    ...
    if (std::find_if(availableTranslations.begin(), availableTranslations.end(),
        ...
        auto *trButter = new QTranslator;
        auto *trQtBase = new QTranslator;
        auto *trQt = new QTranslator;

        const QStringList &butterTrPaths = Butter::getTranslationsDirectories();

        for (const auto &trPath : butterTrPaths) {
            if (trButter
                && trButter->load(locale, QLatin1String("butter"), QLatin1String("_"), trPath)) {
                installTranslator(trButter);
                butterTrLoaded = true;
                trButter = nullptr;
            }
    ...
    if (!butterTrLoaded) {
        qWarning() << "Cannot load Butter's translation for " << locale.name();
    }
    return false;
}

QStringList ButterApplication::getArgs() const
{
...
bool ButterApplication::parseCommandLineOptions()
{
    ...
    // upstream Butter before the rebrand.
    const QCommandLineOption disableButterPlugins({"no-butter-plugins", "no-cutter-plugins"},
                                                  QObject::tr("Do not load Butter plugins"));
    cmdParser.addOption(disableButterPlugins);
    ...
    if (cmdParser.isSet(disablePlugins)) {
        opts.enableButterPlugins = false;
        opts.enableRizinPlugins = false;
    }

    if (cmdParser.isSet(disableButterPlugins)) {
        opts.enableButterPlugins = false;
    }
...
void ButterProxyStyle::polish(QWidget *widget)
{
