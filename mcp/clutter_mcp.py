#!/usr/bin/env python3
"""clutter-mcp -- Model Context Protocol server for Clutter / rizin.

Gives an MCP-speaking AI agent the whole Clutter backend: analyse a binary, decompile
...
Standard library only. It drives the `rizin.exe` that ships next to Clutter, so no GUI
...
LOG_LEVEL = os.environ.get("CLUTTER_MCP_LOG", "1") != "0"


def log(msg):
    if LOG_LEVEL:
        sys.stderr.write("[clutter-mcp] %s\n" % msg)
        sys.stderr.flush()
...
class RizinSession:
    ...
    def start(self, path, debug=False, write=False, arch=None, bits=None,
        ...
        self.cmd("echo __clutter_ready__", timeout=30)
        if analysis:
    ...
    def cmd(self, command, timeout=DEFAULT_TIMEOUT, drop_output=False, limit=None):
        ...
        marker = "__CLUTTER_MARK_%d__" % time.time_ns()
        with self._cv:
            self._buf.clear()
...
def find_rizin(explicit=None):
    for cand in (explicit, os.environ.get("CLUTTER_RIZIN"),
                 os.path.join(REPO, "clutter-dist", "rizin.exe"),
                 os.path.join(REPO, "clutter-dist", "bin", "rizin.exe"),
                 os.path.join(REPO, "clutter-dist", "rizin"),
                 shutil.which("rizin")):
...
def t_open(a):
    ...
    if STATE["rizin"] is None:
        raise ToolError("rizin not found. Set CLUTTER_RIZIN or build Clutter first.")
    with _lock:
...
def prompt_text(name, args):
    ...
    if name == "triage":
        return ("Triage %s with the clutter tools and report:\n"
    ...
    if method == "initialize":
        ...
        return {
            ...
            "serverInfo": {"name": SERVER_NAME, "version": SERVER_VERSION,
                           "title": "Clutter reverse engineering"},
            "instructions": (
                "Drive Clutter/rizin: `open` a binary, `analyze` deep, then `decompile`, "
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="clutter-mcp: MCP server for Clutter/rizin")
